...
def merge_sap_env_data_site(output_file):
    ...
    site_info = pd.read_csv('data/raw/grided/spatial_features/site_info_with_env_data2.csv')
    count = 0
    site_biome_mapping = {}  # Dictionary to keep track of which biome each site belongs to
    
    for sap_data_file in Path('./outputs/processed_data/sap/gap_filled_size1_after_filter').rglob("*.csv"):
        
        try:
            # Parse location and plant type
            parts = sap_data_file.stem.split("_")
            location_type = '_'.join(parts[:-2])
            # if location_type not in good_sites:
                # continue
            
            # Load data files
            env_data_file = Path('./outputs/processed_data/env/gap_filled_size1_after_filter') / f"{location_type}_gap_filled.csv"
            ...
            try:
                ...
                ele = site_info[site_info['site_name'] == location_type]['elevation_m'].values[0]
                slope = site_info[site_info['site_name'] == location_type]['slope_degrees'].values[0]
                aspect_sin = site_info[site_info['site_name'] == location_type]['aspect_sin'].values[0]
                aspect_cos = site_info[site_info['site_name'] == location_type]['aspect_cos'].values[0]
                mean_annual_temp = site_info[site_info['site_name'] == location_type]['bio1'].values[0]
                mean_annual_precip = site_info[site_info['site_name'] == location_type]['bio12'].values[0]
                temp_seasonality = site_info[site_info['site_name'] == location_type]['bio4'].values[0]
                precip_seasonality = site_info[site_info['site_name'] == location_type]['bio15'].values[0]
            ...
            if ele is None or np.isnan(ele):
                print(f"Missing elevation data for {location_type}")
...
def main():
    ...
    output_file = Path('./outputs/processed_data/merged/site/gap_filled_size1_hourly_after_filter') 
