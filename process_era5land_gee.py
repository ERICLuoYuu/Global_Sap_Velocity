...
class ERA5LandGEEProcessor:
    ...
    def create_prediction_dataset(self, start_date=None, end_date=None, points=None, region=None, use_existing_data=True, use_utc=True):
        ...
        pandas.DataFrame or list of pandas.DataFrame
            DataFrame with features ready for prediction (single DF for grid, list for points)
        """
        # --- Setup and Data Loading/Validation (largely unchanged) ---
        if region is None:
            region = {
                'lat_min': config.DEFAULT_LAT_MIN, 'lat_max': config.DEFAULT_LAT_MAX,
                'lon_min': config.DEFAULT_LON_MIN, 'lon_max': config.DEFAULT_LON_MAX
            }

        if use_existing_data:
            if not self.datasets:
                raise ValueError("use_existing_data=True but no data has been loaded.")
            # Determine time range from existing data if needed (unchanged)
            # ... (existing logic to find min/max dates) ...
            # Check required derived variables (unchanged)
            ...
            # Traditional mode - load data (unchanged)
            # ... (existing logic to load data month by month) ...
            pass # Keep existing logic here

        # Convert dates to datetime objects if they're strings (unchanged)
        if start_date: start_date = pd.to_datetime(start_date, utc=use_utc)
        if end_date: end_date = pd.to_datetime(end_date)
        # Apply timezone standardization if needed (unchanged)
        # ...

        # Select only the variables needed for prediction (unchanged)
        prediction_vars = []
        for era5_name in config.REQUIRED_VARIABLES_PREDICTION:
            if era5_name in self.datasets:
                prediction_vars.append(era5_name)

        if not prediction_vars:
            print("No valid data variables found for prediction")
            return None
        print(f"Using variables for prediction: {prediction_vars}")
        print(f"--- Debug Hour Start ---") # Add identifier for hourly logs
        print(f"Input self.datasets keys: {list(self.datasets.keys())}")
        print(f"Prediction variables requested: {prediction_vars}")
                # --- Processing Logic ---
        if points:
            # --- POINTS-BASED APPROACH (Unchanged from original logic) ---
            print("Processing specific points...")
            all_data = []
            # Vectorized climate data lookup for all points (unchanged)
            # ... (get temps, precips for all points) ...
            climate_data_available = True # or False based on success
            ...
            for i, point in enumerate(points):
                ...
                # Get climate data for this point (unchanged)
                # ... (assign temp/precip from batch or fallback) ...

                # Extract time series for each variable (unchanged)
                # ... (loop through prediction_vars, ds.sel(), create series) ...
                # ... (store series in point_data[model_name]) ...

                # Create DataFrame from the point data (unchanged)
                try:
                    all_series = [s for s in point_data.values() if isinstance(s, pd.Series)]
                    if all_series:
                        # Combine series for this point
                        # Ensure consistent index before creating DataFrame
                        common_index = None
                        for s in all_series:
                            if common_index is None:
                                common_index = s.index
                            elif not common_index.equals(s.index):
                                print(f"Warning: Inconsistent time index found for point {point_data['name']}. Attempting reindex.")
                                # Optional: Decide how to handle inconsistent indices (e.g., reindex, error)
                                # Example: Reindex to the first series' index
                                s_reindexed = s.reindex(common_index, method='nearest') # Or other method
                                point_data[s.name] = s_reindexed # Update dict with reindexed series

                        df = pd.DataFrame(point_data)

                        # Set datetime index if available from series
                        if common_index is not None:
                           df.index = common_index
                           df.index.name = 'timestamp' # Ensure index has a name

                        # Add non-series data (lat, lon, name, climate)
                        for key, value in point_data.items():
                            if not isinstance(value, pd.Series):
                                df[key] = value # Broadcast scalar values
                        ...
                        if config.TIME_FEATURES:
                           if isinstance(df.index, pd.DatetimeIndex):
                               ...
                               print(f"Warning: Cannot add time features for point {point_data['name']} due to non-datetime index.")

                        # Reset index to have timestamp as a column if needed, or keep it as index
                        df = df.reset_index()
                        all_data.append(df)
                    else:
            ...
            # 1. Identify coordinate names from the first dataset
            first_ds = self.datasets[prediction_vars[0]]
            lat_var, lon_var, time_var = None, None, None
            for var in first_ds.coords:
                if var.lower() in ['latitude', 'lat']: lat_var = var
                elif var.lower() in ['longitude', 'lon']: lon_var = var
                elif var.lower() in ['time', 'datetime']: time_var = var

            if not all([lat_var, lon_var, time_var]):
                print("Could not identify standard coordinate variables (latitude, longitude, time)")
                return None

            lats = first_ds[lat_var].values
            lons = first_ds[lon_var].values
            # Ensure times are standardized (using the first dataset's time)
            times = self.standardize_timestamps(pd.DatetimeIndex(first_ds[time_var].values), use_utc=use_utc)


            print(f"Grid dimensions: lat={len(lats)}, lon={len(lons)}, time={len(times)}")

            # 2. Get Climate Data (Vectorized - assuming this part worked as per user)
            ...
            if hasattr(self, 'temp_climate_data') and hasattr(self, 'precip_climate_data'):
                try:
                    ...
                    print("Successfully retrieved climate data for grid") # <<< User confirmed code passes this
                except Exception as e:
                    print(f"Error in grid climate data retrieval: {str(e)}")
                    # Fallback: Create grids of default values
                    temp_grid = np.full((len(lats), len(lons)), 15.0)
                    precip_grid = np.full((len(lats), len(lons)), 800.0)
            else:
                 print("Climate data files not loaded, using default values.")
                 temp_grid = np.full((len(lats), len(lons)), 15.0)
                 precip_grid = np.full((len(lats), len(lons)), 800.0)


            # 3. Merge all variable datasets into one xarray Dataset
            print("Merging variable datasets...")
            datasets_to_merge = []
            common_coords = {lat_var: lats, lon_var: lons, time_var: times}

            for var_name in prediction_vars:
                ds = self.datasets[var_name]
                 # Ensure dataset uses standardized time coordinate name and values
                current_time_var = None
                for dim in ds.dims:
                     if dim.lower() in ['time', 'datetime']:
                         current_time_var = dim
                         break
                if current_time_var and current_time_var != time_var:
                     ds = ds.rename({current_time_var: time_var})

                # Select the time range AFTER standardization
                if start_date is not None and end_date is not None:
                    # Ensure start/end dates match the timezone awareness of 'times'
                    std_start = self.standardize_timestamps(pd.DatetimeIndex([start_date]), use_utc=use_utc)[0]
                    std_end = self.standardize_timestamps(pd.DatetimeIndex([end_date]), use_utc=use_utc)[0]
                    ds = ds.sel({time_var: slice(std_start, std_end)})


                # Reindex to ensure perfect alignment (optional but safer)
                # This might be slow if datasets are very large and not aligned
                # ds = ds.reindex(common_coords, method='nearest', tolerance='1S') # Adjust tolerance as needed
                datasets_to_merge.append(ds)

            try:
                # Use join='override' if variables might have slightly different coords after selection
                # Use compat='override' if attributes differ but data is compatible
                merged_ds = xr.merge(datasets_to_merge, join='override', compat='override')
            except Exception as merge_err:
                 print(f"Error merging datasets: {merge_err}")
                 print("Attempting merge with minimal checks (compat='no_conflicts')...")
                 try:
                     merged_ds = xr.merge(datasets_to_merge, compat='no_conflicts')
                 except Exception as final_merge_err:
                     print(f"Final merge attempt failed: {final_merge_err}")
                     return None # Cannot proceed if merge fails


            # 4. Add climate data as DataArrays
            print("Adding climate data to merged dataset...")
            merged_ds['annual_mean_temperature'] = xr.DataArray(
                temp_grid, dims=[lat_var, lon_var], coords={lat_var: lats, lon_var: lons}
            )
            merged_ds['annual_precipitation'] = xr.DataArray(
                precip_grid, dims=[lat_var, lon_var], coords={lat_var: lats, lon_var: lons}
            )
            # ... after the xr.merge call ...
            print(f"Variables in merged_ds BEFORE rename: {list(merged_ds.data_vars)}")
            # 5. Rename variables to model names
            print("Renaming variables...")
            rename_dict = {}
            final_vars = ['annual_mean_temperature', 'annual_precipitation'] # Start with climate vars
            for era5_name, model_name in config.VARIABLE_RENAME.items():
                if era5_name in merged_ds:
                    rename_dict[era5_name] = model_name
                    final_vars.append(model_name)
            print(f"Rename dictionary created: {rename_dict}")
            print(f"Final variables list to keep: {final_vars}")

            merged_ds = merged_ds.rename(rename_dict)
            # ... after the loop building rename_dict ...
            print(f"Variables in merged_ds AFTER rename: {list(merged_ds.data_vars)}")
            # Keep only the final variables needed (original + climate)
            merged_ds = merged_ds[final_vars]

            print(f"Variables in merged_ds AFTER filtering: {list(merged_ds.data_vars)}")
            # 6. Convert the entire xarray Dataset to a Pandas DataFrame
            # This is the most efficient way to get the desired structure
            print("Converting merged xarray Dataset to Pandas DataFrame...")
            # This operation performs the reshaping and can be memory intensive
            try:
                # Ensure chunks are suitable for to_dataframe if using Dask
                # merged_ds = merged_ds.chunk({'time': -1}) # Example: Chunk fully along time

                # to_dataframe() creates a MultiIndex (time, lat, lon)
                final_df = merged_ds.to_dataframe()

                # Check for and handle potential all-NaN slices resulting from merge/reindex
                # final_df = final_df.dropna(subset=final_vars, how='all') # Drop rows where ALL data vars are NaN

            except Exception as df_conv_err:
                 print(f"Error converting to DataFrame: {df_conv_err}")
                 print("This might be a memory issue for large datasets.")
                 # Optional: Implement chunked conversion if memory is the issue
                 # e.g., iterate through time chunks of merged_ds and convert/save parts
                 return None


            # The index is now (time, latitude, longitude) or similar
            # Reset the index to get time, lat, lon as columns
            final_df = final_df.reset_index()

            # Rename coordinate columns to standard names if needed
            final_df = final_df.rename(columns={time_var: 'timestamp', lat_var: 'latitude', lon_var: 'longitude'})


            # 7. Add Time Features (if configured)
            if config.TIME_FEATURES:
                ...
                if 'timestamp' in final_df.columns:
                    ...
                    print("Warning: 'timestamp' column not found, cannot add time features.")

            # 8. Add Name column (optional, based on original logic)
            print("Adding identifier columns...")
            final_df['name'] = 'Grid_' + final_df['latitude'].astype(str) + '_' + final_df['longitude'].astype(str)
