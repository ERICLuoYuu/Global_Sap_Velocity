...
TIMESTAMP_NAMES = ['timestamp', 'Hour', 'month', 'date', 'datetime', 'date.time', 'date_utc+1', 

                   'time', 'datum', 'fecha', 'data', 'timestamp_utc', 'doy',

                   'date_time', 'measurement_time', 'observation_time']

TREE_ID_NAMES = ['tree_id', 'treeid', 'treenumber', 'tree_number', 'treenumb', 
                 'treenumber/treen', 'treen',  # CH-Dav uses 'TreeNumber/TreeN'
                 'tid', 'tree', 'plant_id', 'plantid', 'plot', 'tree_code',
                 'sample_id', 'series' ]

# Expanded list to catch more column patterns

VALUE_NAMES = ['value', 'sapflow','sap','sfd', 'sfdm', 'spd', 'flow', 'flux', 'tdsv',

               'tree_sapflow_kg/h', 'Js_outer', 'Js_inner', 'velocity', 'rate',

               'js', 'sap_flow', 'sap_velocity', 'transpiration', 'water_flow',

               'flow_rate', 'flux_density', 'sap_flux', 'sf', 'vfd', 'measurement']

ENV_NAMES = ['temperature', 'Ta', 'Tair','air_temperature', 'temp', 'RH', 'rH', 'relative_humidity', 'precipitation', 'precipitations(mm)', 'precip', 

             'evapotranspiration', 'Et', 'Evapotransp. (mmol m-2s-1)', 'soil_moisture', 'sm', 'SoilUR10cm', 'SoilUR50cm', 'SoilUR100cm', 'humidity', 'wind_speed', 'ws', 'vapor_pressure_deficit', 'vpd',

             'shortwave_incoming_radiation', 'sw_in', 'solar_radiation', 'ppfd', 'photosynthetic_photon_flux_density', 'ppfd_in', 'gpp (umol m-2s-1)', 'gpp']

# --- CONFIGURATION --- (add to existing)

SENSOR_ID_NAMES = ['orientation', 'sensor', 'probe_id', 'probeid', 'probe',

                   'probe_number']

# --- END CONFIGURATION ---

def extract_tree_id_from_column(col_name: str) -> Optional[str]:
...
def extract_sensor_id_from_column(col_name: str) -> Optional[str]:
...
def build_id_based_column_mapping(value_cols: List[str], meta_df: pd.DataFrame) -> Dict[str, str]:
    ...
    if unmatched:
        ...
        print(f"        WARNING: {len(unmatched)} columns could not be matched by ID: {unmatched[:3]}{'...' if len(unmatched) > 3 else ''}")
...
def detect_delimiter(file_path: Path, sample_lines: int = 5) -> str:
    ...
    try:
        ...
        delimiter_counts = {delim: sum(line.count(delim) for line in lines) 
...
def read_data_file(file_path: Path, header='infer') -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        print(  f"        INFO: Successfully read file '{file_path.name}' with shape {df.shape}, data columns: {list(df.columns)}")
        ...
        if header == 'infer' or header == 0:
            ...
            meaningful_count = sum(1 for col in col_names_str 
            ...
            for col in cols_to_check:
                ...
                if n_valid > len(df) * 0.1:  # If more than 10% are valid numeric values
                    ...
                    print(f"        INFO: Column '{col}' appears meaningless but contains {n_valid} numeric values - KEEPING IT!")
        ...
        if not has_meaningful_headers:
            ...
            if 'Unnamed: 0' in df.columns and 'A' in df.columns:
                ...
                if a_col_numeric.notna().sum() == 0:  # All non-numeric
                    ...
                    print(f"        INFO: Column 'A' contains no numeric data - dropping it")
                    ...
                    print(f"        INFO: Renamed 'Unnamed: 0' to 'value' (contains the actual data)")

            else:

                print(f"        INFO: No special case detected")
                ...
                if n_cols == 1:
                    ...
                    print(f"        INFO: Single column detected, assigned as 'value' (timestamp to be generated)")
                    ...
                    print(f"        INFO: Two columns detected, assigned as ['timestamp', 'value']")
        ...
        if not df.empty and len(df.columns) > 0 and 'timestamp' in df.columns:
            ...
            print(f"        DEBUG READ: Found time column: 'timestamp'")
...
def tidy_metadata(site_path: Path, metadata_file: str, site_code: str, 

                  data_files: List[str] = []) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if site_sheet_name and sapflow_sheet_name:
            ...
            site_df_wide = all_sheets[site_sheet_name].set_index(

                all_sheets[site_sheet_name].columns[0])

            site_df_wide = site_df_wide.loc[~site_df_wide.index.duplicated(keep='first')]

            

            plant_info = all_sheets[sapflow_sheet_name].set_index(

                all_sheets[sapflow_sheet_name].columns[0])
    ...
    if meta_df_tidy is None:
        ...
        try:
            ...
            meta_df_tidy = meta_df_wide.loc[~meta_df_wide.index.duplicated(keep='first')].transpose().reset_index(drop=True)
    ...
    if site_id is None:
        ...
        site_id = next((col for col in meta_df_tidy.columns if 'site' in str(col).lower() and 'id' in str(col).lower()), None)

    # Last resort - any column just containing 'site' (but not 'site_' prefixed columns like 'site_stems')

    if site_id is None:

        site_id = next((col for col in meta_df_tidy.columns 

                       if str(col).lower().strip() == 'site' or str(col).lower().startswith('site_id')), None)
    ...
    if tree_id is None:
        tree_id = next((col for col in meta_df_tidy.columns 
                       if ('tree' in str(col).lower() and ('id' in str(col).lower() or 'number' in str(col).lower() or 'numb' in str(col).lower()))
                       or 'plant' in str(col).lower()), None)
    ...
    if site_id not in meta_df_tidy.columns or tree_id not in meta_df_tidy.columns:
        ...
        print(f"    ERROR: Required columns 'site_id' or 'tree_id' not found in metadata")
    ...
    if 'sensor_id' in meta_df_tidy.columns:
        ...
        meta_df_tidy['pl_code'] = meta_df_tidy.apply(
            lambda r: f"{r[site_id]}-{r[tree_id]}", axis=1)
...
def find_col_by_names(df_columns: List[str], potential_names: List[str]) -> Optional[str]:
    ...
    for col_lower, col_orig in df_cols_lower.items():
        ...
        if is_env:
            ...
            if any(kw in col_lower for kw in ['treeid', 'tree_id', 'treenumber', 'tree_number', 'tree number', 

                                               'sensorid', 'sensor_id', 'sensor', 'probe', 'orientation']):

                continue

            if any(kw in col_lower for kw in ['sapflow', 'sap flow', 'sap_flow', 'sap flux', 'sap_flux',
        ...
        if is_value:
            ...
            if any(pattern in col_lower for pattern in ['tdsv', 'td1', 'td2', 'td3', 'td4', 'td5', 
                ...
                if any(part in ['tdsv1', 'tdsv2', 'tdsv3', 'tdsv4', 'tdsv5', 'tdsv6', 'tdsv7', 'tdsv8', 'tdsv9', 'tdsv10',

                               'td1', 'td2', 'td3', 'td4', 'td5', 'td6', 'td7', 'td8', 'td9', 'td10',

                               'k1', 'k2', 'k3', 'k4', 'k5', 'k6', 'k7', 'k8', 'k9', 'k10'] for part in col_parts):
            ...
            if any(kw in col_lower for kw in ['timestamp', 'date', 'datetime', 'datum', 'fecha', 'doy', 'toy', 'hour', 'month']):
            ...
            if any(kw in col_lower for kw in ['treeid', 'tree_id', 'treenumber', 'tree_number',

                                               'sensorid', 'sensor_id', 'sensor number', 'probe']):

                if not any(vk in col_lower for vk in ['sapflow', 'kg/h', 'g h-1', 'flux', 'sfd', 'flow', 'js_', 'js_outer', 'js_inner']):
        ...
        if is_tree_id or is_sensor_id:
            ...
            if any(term in col_lower for term in ['sapflow', 'kg/h', 'flux', 'velocity', 'sfd', 'flow', 'js_', 'js_outer', 'js_inner']):
        ...
        for keyword in potential_names:
            ...
            elif keyword_lower in col_lower.split('_') or keyword_lower in col_lower.split('-') or keyword_lower in col_lower.split('.'):
    ...
    if not matches:
        ...
        if is_value:
            ...
            for col_lower, col_orig in df_cols_lower.items():
                ...
                if any(kw in col_lower for kw in ['timestamp', 'date', 'datetime', 'treeid', 'tree_id',

                                                   'sensorid', 'sensor_id', 'sensor', 'probe', 'doy', 'toy', 'hour', 'month',

                                                   'tdsv', 'td1', 'td2', 'td3', 'td4', 'td5', 'td6', 'td7', 'td8', 'td9', 'td10',
            ...
            if potential_value_cols:
                ...
                if is_value:
                    ...
                    print(f"        DEBUG: Found value columns (fallback): {potential_value_cols[:3]}{'...' if len(potential_value_cols) > 3 else ''}")
...
def process_pattern_A(site_path, site_code, metadata_file, data_files, output_path):
    ...
    if 'sensor_id' in meta_df_tidy.columns:
        ...
        sensor_id_to_pl_code = pd.Series(meta_df_tidy.pl_code.values, 

                                         index=meta_df_tidy.sensor_id.astype(str)).to_dict()
    ...
    for base_name, file_list in file_groups.items():
        ...
        for f in file_list:
            ...
            print(f"        DEBUG: File '{f}' has columns: {list(df.columns)[:5]}{'...' if len(df.columns) > 5 else ''}")
        ...
        ts_col = None
        print(f"        DEBUG: needs_timestamp_generation={needs_timestamp_generation}, columns={full_plant_df.columns.tolist()}")
        ...
        print(f"        DEBUG DETECT: ts_col={ts_col}, tree_col={tree_col}, sensor_col={sensor_col}, value_col={value_col}")
        ...
        if sensor_col:
            ...
            if len(unique_sensor_values) <= 1:
                ...
                print(f"    INFO: Ignoring sensor_col '{sensor_col_name}' - uninformative (all values = '{uninformative_val}')")
        ...
        if tree_col and sensor_col:
            ...
            # Both exist - combine them for unique identifiers

            print(f"    INFO: Both tree_col and sensor_col found. Combining for unique identifiers.")
        ...
        if isinstance(ts_col, list):
            ...
            if len(ts_col) > 1:
                        ...
                        for col in ts_col:
                            ...
                            if len(valid_timestamps) > 1:
                                ...
                                if mode_diff and abs(mode_diff - expected_freq) < pd.Timedelta(minutes=1):
                                    ...
                                    print(f"    INFO: Selected timestamp column '{col}' matching data frequency '{freq_str}'")
                                    ...
                                    print(f"        DEBUG: Column '{col}' mode diff {mode_diff} does not match expected {expected_freq}")
                ...
                if not best_ts_col and 'Date' in ts_col and 'Time' in ts_col:
                    ...
                    print(f"    INFO: Combining Date and Time columns...")

                    print(f"        DEBUG: ts_col = {ts_col}")

                    print(f"        DEBUG: Checking if 'Date' and 'Time' columns exist in dataframe...")
                    ...
                    try:
                        ...
                        if has_hour_attr:  # datetime.time objects
                            ...
                            print(f"        DEBUG: Converting datetime.time objects to timedelta...")
                            ...
                            time_deltas = time_values.apply(
                                ...
                                if pd.notna(t) and hasattr(t, 'hour') else None
                            ...
                            print(f"        DEBUG: Combined timestamp first 3: {full_plant_df['combined_timestamp'].head(3).tolist()}")

                        else:

                            print(f"        DEBUG: Parsing time as strings...")
                            ...
                            full_plant_df['combined_timestamp'] = pd.to_datetime(
                                ...
                                dates.dt.date.astype(str) + ' ' + times.dt.time.astype(str),
                        ...
                        if len(valid_timestamps) > 10:
                            ...
                            print(f"    INFO: âœ“ Successfully combined Date+Time! ({len(valid_timestamps)} valid timestamps)")
                            ...
                            if expected_freq:
                                ...
                                if mode_diff:
                                    ...
                                    print(f"        DEBUG: Combined timestamp frequency: {mode_diff} (expected: {expected_freq})")

                        else:

                            print(f"        WARNING: Only {len(valid_timestamps)} valid timestamps - need more than 10!")
                ...
                if best_ts_col:
                    ...
                    print(f"    INFO: No valid timestamp column found, generating from metadata...")
        ...
        if env_cols and ts_col:
            ...
            print(f"    INFO: Found {len(env_cols)} environmental column(s): {env_cols[:3]}{'...' if len(env_cols) > 3 else ''}")
        ...
        if not id_col:
            ...
            print(f"        DEBUG BRANCH: Entered 'no id_col' branch (simple file group)")
            ...
            try:
                ...
                if isinstance(value_col, list):
                    ...
                    print(f"        DEBUG BRANCH: value_col is a list, entering list processing branch")
                    ...
                    print(f"        DEBUG: Before rename - columns: {list(full_plant_df.columns)}, shape: {full_plant_df.shape}")
                    ...
                    print(f"        DEBUG: After timestamp parse - valid timestamps: {df['timestamp'].notna().sum()}/{len(df)}")
                    ...
                    for col in renamed_vc:
                        ...
                        print(f"        DEBUG: Converting column '{col}' to numeric. Sample values before: {df[col].head(3).tolist()}")
                    ...
                    if not df.empty:
                        ...
                        print(f"        DEBUG: DataFrame is empty after processing!")

                

                else:  # Single value column

                    print(f"        DEBUG BRANCH: Entered ELSE block for single value column")
                    ...
                    print(f"        DEBUG: After selecting and renaming - df.shape={df.shape}, df.columns={list(df.columns)}")
                    ...
                    print(f"        DEBUG: Converting column '{pl_code}' to numeric. Sample values before: {df[pl_code].head(3).tolist()}")

                    df[pl_code] = pd.to_numeric(df[pl_code], errors='coerce')

                    print(f"        DEBUG: After conversion - valid values: {df[pl_code].notna().sum()}/{len(df)}, NA values: {df[pl_code].isna().sum()}")
            ...
            id_col_display = id_col if not use_combined_id else f"combined({tree_col_for_lookup}+{sensor_col_for_lookup})"
                ...
                if not long_df.empty:
                    ...
                    if use_combined_id:
                        ...
                        for col in pivoted_df.columns:
                            ...
                            if len(parts) >= 2:
                                ...
                                try:
                                    ...
                                    elif tree_col_for_lookup in meta_df_tidy.columns and sensor_col_for_lookup in meta_df_tidy.columns:
                        ...
                        if len(value_col) > 1:
                            ...
                            pivoted_df.columns = [f"{id_to_pl_code.get(str(tid), str(tid))}-{sensor_suffix}" 

                                                for tid in pivoted_df.columns]

                        else:

                            pivoted_df.columns = [id_to_pl_code.get(str(tid), str(tid)) 
        ...
        if processed_df is not None and not processed_df.empty:
            ...
            print(f"        DEBUG FINAL: processed_df is valid, shape={processed_df.shape}, appending to all_sapflow_dfs")
...
def process_pattern_B(site_path, site_code, metadata_file, data_files, output_path):
    ...
    for base_name, file_list in file_groups.items():
        ...
        if ts_col is None:
            ...
            print(f"        INFO: No timestamp column found. Attempting to generate from metadata...")
        ...
        if ts_col.upper() == 'DOY' or 'doy' in ts_col.lower():
            ...
            print(f"        INFO: Detected DOY (Day of Year) format. Constructing timestamps...")
            ...
            if '_file_year' not in combined_df.columns:
                ...
                print(f"    ERROR: DOY format detected but no year information available. Skipping.")
            ...
            try:
                ...
                if hour_col:
                    ...
                    print(f"        INFO: Constructed timestamps from Year + DOY")
        ...
        if value_cols is None:
            ...
            exclude_cols = ['timestamp', '_file_year', ts_col] + (env_cols if env_cols and isinstance(env_cols, list) else [])
            ...
            print(f"        INFO: Using all remaining columns as values: {value_cols[:5]}{'...' if len(value_cols) > 5 else ''}")
        ...
        print(f"        INFO: Attempting ID-based column mapping...")
        ...
        if column_mapping:
            ...
            print(f"        WARNING: ID-based mapping failed, falling back to order-based mapping")
            ...
            if len(value_cols) != len(plant_codes):
                ...
                print(f"    WARNING: Number of value columns ({len(value_cols)}) doesn't match number of plants in metadata ({len(plant_codes)})")

                print(f"    INFO: Will map available columns to plant codes in order")
        ...
        if not sapflow_df.empty:
            ...
            print(f"        INFO: âœ“ Processed sapflow data - shape: {sapflow_df.shape}, columns: {list(sapflow_df.columns)[:5]}{'...' if len(sapflow_df.columns) > 5 else ''}")

        else:

            print(f"        WARNING: Sapflow dataframe is empty after processing")
    ...
    print(f"    INFO: Final sapflow dataframe - shape: {site_sapflow_df.shape}, columns: {list(site_sapflow_df.columns)[:5]}{'...' if len(site_sapflow_df.columns) > 5 else ''}")
...
def process_pattern_C(site_path, site_code, metadata_file, data_files, output_path):
    ...
    tree_id_to_pl_code = pd.Series(

        meta_df_tidy.pl_code.values, 

        index=meta_df_tidy.tree_id.astype(str)
    ...
    for group_key, file_list in file_groups.items():
        ...
        for file in file_list:
            ...
            if df.empty:
                ...
                print(f"        WARNING: No valid timestamps. Skipping file.")
            ...
            if not id_col:
                ...
                print(f"        WARNING: No ID column found. Skipping file.")
            ...
            if not value_col:
                ...
                print(f"        WARNING: No value column found. Skipping file.")
            ...
            if long_df.empty:
                ...
                print(f"        WARNING: No valid data after cleaning. Skipping file.")
            ...
            try:
                ...
                if use_combined_id:
                    ...
                    print(f"        INFO: Using combined ID mapping for columns")
                    ...
                    print(f"        INFO: Using simple ID mapping for columns")
                    ...
                    if tree_col:
                        ...
                        pivoted_df.columns = [
                            ...
                            tree_id_to_pl_code.get(str(tid), f"{site_code}-{tid}")
...
def write_sfn_files(meta_df: pd.DataFrame, sapflow_df: pd.DataFrame, 

                    site_code: str, output_path: Path, env_df: Optional[pd.DataFrame] = None):
    ...
    RENAME_MAP = {
        ...
        'site_id': 'si_code', 'site': 'si_code','country': 'si_country', 'site_latitude': 'si_lat',

        'site_longitude': 'si_long', 'site_altitude': 'si_elev', 

        'site_affliation': 'si_contact_institution', 'site_affiliation': 'si_contact_institution',

        'contacting_person': 'si_contact_name', 'contact_person': 'si_contact_name',

        'corresponding_flux_tower': 'si_flux_tower', 'flux_tower': 'si_flux_tower',

        'notes': 'si_remarks', 'other_notes': 'si_other_remarks',

        

        # Stand Metadata

        'site_forest_type': 'st_forest_type', 'forest_type': 'st_forest_type',

        'site_soil_type': 'st_soil_type', 'soil_type': 'st_soil_type',

        'site_stems_per_hectare': 'st_density', 'stems_per_hectare': 'st_density',

        'site_basal_area': 'st_basal_area', 'basal_area': 'st_basal_area',

        'site_dominant_tree_height': 'st_height', 'dominant_tree_height': 'st_height',

        'site_leaf_area_index': 'st_lai', 'leaf_area_index': 'st_lai', 'lai': 'st_lai',

        

        # Plant Metadata

        'tree_id': 'pl_name', 'tree': 'pl_name','TreeNumber/TreeNumber': 'pl_name',

        'tree_species': 'pl_species', 'species': 'pl_species', 'species': 'pl_species',

        'tree_dbh': 'pl_dbh', 'dbh': 'pl_dbh', 'DBH (cm)': 'pl_dbh',

        'tree_height': 'pl_height', 'height': 'pl_height', 'Height (m)': 'pl_height',

        'tree_age': 'pl_age', 'age': 'pl_age',

        'tree_status': 'pl_social', 'status': 'pl_social',

        'tree_totalbark_thickness_mm': 'pl_bark_thick', 'bark_thickness': 'pl_bark_thick',

        'tree_sapwood_thickness_cm': 'pl_sapw_depth', 'sapwood_depth': 'pl_sapw_depth',

        'tree_sapwood_area': 'pl_sapw_area', 'sapwood_area': 'pl_sapw_area', 'tree_sapwood_area_cm2': 'pl_sapw_area',

        'sensor_type': 'pl_sens_meth', 'sensor': 'sensor_type', 'method': 'pl_sens_meth',

        'sensor_height': 'pl_sens_hgt', 'units': 'pl_sap_units_orig',

        'tree_latitude': 'pl_lat', 'latitude': 'pl_lat',

        'tree_longitude': 'pl_long', 'longitude': 'pl_long',

        'sensor_id': 'pl_sensor_id', 'series_id': 'pl_series_id', 'sensor_start': 'pl_sensor_start',

        'sensor_stop': 'pl_sensor_end', 'sensor_cutout': 'pl_sensor_cutout', 'sensor_exposition': 'pl_sensor_exposition',

        'tree_altitude': 'pl_elev', 'elevation': 'pl_elev', 'tree_genus': 'pl_genus',

        'genus': 'pl_genus', 'tree_phloem_thickness_mm': 'pl_phloem_thick',

        

        # Environmental Metadata

        'sensor_timezone': 'env_time_zone', 'timezone': 'env_time_zone',

        'data_frequency': 'env_timestep', 'frequency': 'env_timestep',
    ...
    meta_df_sfn = meta_df.rename(

        columns=lambda c: str(c).lower().replace(' ', '_')
    ...
    try:
        ...
        elif any(pattern in unit_normalized for pattern in [

            'cm-2', 'cm2', 'cmÂ²', '/cm2', '/cmÂ²', 'cm^-2', 'cm^2', 'per cm2', 'per cmÂ²', 'cmâ»Â²', 'cm â»Â²',

            'm-2', 'm2', 'mÂ²', '/m2', '/mÂ²', 'm^-2', 'm^2', 'per m2', 'per mÂ²',

            'mm-2', 'mm2', 'mmÂ²', '/mm2', '/mmÂ²', 'mm^-2', 'mm^2', 'per mm2', 'per mmÂ²',

            'sapwood area', 'per area', 'per cm ', 'per m ', 'per mm ', 'per sapwood ',

            'cm/h', 'm/h', 'mm/h', 'cm h-1', 'm h-1', 'mm h-1', 'cm/s', 'm/s', 'mm/s', 'cm s-1', 'm s-1', 'mm s-1', 'cm/hour'
    ...
    MAPS = {
        ...
        'site_md': [
            ...
            'si_code', 'si_country', 'si_lat', 'si_long', 'si_elev',

            'si_contact_institution', 'si_contact_name', 'si_flux_tower',

            'si_remarks', 'si_other_remarks'

        ],

        'stand_md': [

            'st_site_id', 'st_forest_type', 'st_soil_type', 'st_density',

            'st_basal_area', 'st_height', 'st_lai', 'st_sand_perc',

            'st_silt_perc', 'st_clay_perc'

        ],

        'plant_md': [

            'pl_code', 'pl_name', 'pl_species', 'pl_genus', 'pl_dbh',

            'pl_height', 'pl_age', 'pl_social', 'pl_bark_thick',

            'pl_phloem_thick', 'pl_sapw_depth', 'pl_sapw_area',

            'pl_lat', 'pl_long', 'pl_elev', 'pl_sens_meth',

            'pl_sens_hgt', 'pl_sensor_id', 'pl_series_id', 'pl_sensor_start',

            'pl_sensor_end', 'pl_sensor_cutout', 'pl_sensor_exposition',

            'pl_sap_units_orig', 'pl_variable_name', 'pl_remarks'
    ...
    for md_type, cols in MAPS.items():
        ...
        if md_type in ['site_md', 'stand_md', 'env_md']:
            ...
            if not md_df.empty and 'pl_code' in meta_df_sfn.columns:
                ...
                md_df = md_df.merge(species_counts, left_on='sp_name', right_index=True).rename(columns={'pl_code': 'sp_ntrees'})
...
def calculate_formatted_site_years(output_dir: str, max_gap_days: int = 366):
    ...
    if not summary_df.empty:
        ...
        for _, row in site_summary.sort_values('site_code').iterrows():
            ...
            print(f"{row['site_code']:<20} | {s_date:<12} | {e_date:<12} | {row['years']:.2f}     | {row['coverage_percent']:.1f}%   | {row['rows_removed']} rows")
...
def main():
    ...
    if not input_root.exists(): print(f"ERROR: Input directory '{INPUT_DIR}' not found."); return

    print(f"Starting SAPFLUXNET formatting...\nInput: '{input_root}'\nOutput: '{output_root}'\n")

    output_root.mkdir(exist_ok=True)

    processor = {'AT_Mmg': process_pattern_A, 
    ...
    for site_code in processor.keys():

        site_path = input_root / site_code

        if not site_path.is_dir(): continue
