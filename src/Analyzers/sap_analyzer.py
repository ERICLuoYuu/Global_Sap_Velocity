...
class SapFlowAnalyzer:
    ...
    def __init__(self, paths: PathConfig = None, scale: str = 'sapwood'):
        ...
        # --- Removal log support ---
        self.removal_processor: Optional[RemovalLogProcessor] = None
        self._removal_log_path: Optional[Path] = None
        self.removal_stats = {
            'sites_skipped': [],
            'columns_removed': {},
            'periods_removed': {},
    ...
    def _apply_removal_log(self, df: pd.DataFrame, location: str, 
                           plant_type: str) -> pd.DataFrame:
        """
        ...
        if site_key not in self.removal_stats['columns_removed']:
            self.removal_stats['columns_removed'][site_key] = \
                self.removal_processor.get_columns_to_remove(location, plant_type)
    ...
    def save_removal_report(self, output_path: Union[str, Path] = None):
        """
    ...
    def run_analysis_in_batches(self, batch_size: int = 1, switch = 'both'):
        ...
        if switch == 'both':
            for i, batch in enumerate(file_batches):
                print("-" * 80)
                print(f"Loading and plotting Batch {i+1}/{len(file_batches)} (files {i*batch_size + 1} to {i*batch_size + len(batch)})")
            ...
            if not self.data_outlier_removed_dir.exists():
                ...
                for i, batch in enumerate(file_batches_outliersremoved):
                    print("-" * 80)
                    print(f"Plotting Batch {i+1}/{len(file_batches)} (files {i*batch_size + 1} to {i*batch_size + len(batch)})")
                    print("-" * 80)
                    print(f"Plotting all plants from the loaded data (up to {len(all_sapf_files)} files)")
                    self.plot_all_plants(save_dir=self.paths.sap_cleaned_figures_dir, files_to_load=batch, progress_update=False)
                    self._clear_batch_data()
        else:
    ...
    def calculate_site_years(self, file_pattern: str = "*_sapf_data.csv", max_gap_days: int = 366) -> pd.DataFrame:
        ...
        for file in all_files:
            try:
                ...
                length_raw = len(df)
                df = df.dropna(how='all', subset=[col for col in df.columns if col != 'TIMESTAMP' and col != 'solar_TIMESTAMP'])
    ...
    def _load_sapflow_data(self, files_to_load: List[Path]):
        ...
        for file in files_to_load:
            ...
            try:
                ...
                df_processed = self._process_sapflow_data(df, latitude, longitude, timestep, time_zone, elevation, location, plant_type, flags)
    ...
    def _process_sapflow_data(self, df: pd.DataFrame, latitude: float, longitude: float, 
                               timestep: float, time_zone: str, elevation: float, 
                               location: str = None, plant_type: str = None, 
                               flags: pd.DataFrame = None) -> pd.DataFrame:
        """
        ...
        SAP_FLOW_VARIABLES = [col for col in df.columns if col not in ['TIMESTAMP', 'solar_TIMESTAMP', 'TIMESTAMP_LOCAL', 'lat', 'long'] and 'Unnamed:' not in str(col)]
        
        # Convert numeric columns to float, with error handling
        numeric_cols = [col for col in df.columns if col != 'TIMESTAMP' and col != 'solar_TIMESTAMP' and col != 'TIMESTAMP_LOCAL']
        ...
        try:
            ...
            data_cols = [col for col in df.columns if col not in ['solar_TIMESTAMP', 'TIMESTAMP_LOCAL', 'lat', 'long'] and 'Unnamed:' not in str(col)]
            
            variability_stats = []
            
            # Calculate day/night mask ONCE for all columns
            try:
                day_mask = self._calculate_daytime_mask(df.index, current_df_latitude, current_df_longitude, elevation = 0, elevation_threshold=0)
            ...
            for col in data_cols:
                ...
                local_time_series = df['solar_TIMESTAMP'] if 'solar_TIMESTAMP' in df.columns else pd.Series(df.index, index=df.index)
                ...
                if not day_series.dropna().empty:
                    print(f"  Processing daytime outliers...")
                    day_outliers = self._detect_outliers(
                        series=day_series,
                        n_std=DAY_SAPFLOW_N_STD,
                        time_window=DAY_SAPFLOW_WINDOW_POINTS,
                        method='B'
                    )
                    final_col_outliers.loc[day_outliers[day_outliers].index] = True
                
                if not night_series.dropna().empty:
                    print(f"  Processing nighttime outliers...")
                ...
                print(f"\n  Summary for {col}:")
                print(f"    Outliers: {final_col_outliers.sum()} ({final_col_outliers.sum()/len(column_series)*100:.2f}%)")
                print(f"    Variability: {variability_mask.sum()} ({variability_mask.sum()/len(column_series)*100:.2f}%)")
                print(f"    Total removed: {final_col_outliers.sum() + variability_mask.sum()} ({(final_col_outliers.sum() + variability_mask.sum())/len(column_series)*100:.2f}%)")
            ...
            # ====================================================================
            print(f"\n{'='*60}")
            print(f"[5/5] Incomplete Day Removal (>50% missing daytime data)")
            print(f"{'='*60}")
    ...
    def _correct_baseline_drift(self, series: pd.Series, day_mask: pd.Series, 
        ...
        if method == 'nighttime_quantile':
            ...
            baseline = night_values.rolling(
                ...
                center=True, 
                min_periods=max(1, window_points//4)
            ).quantile(quantile)
    ...
    def _detect_and_remove_reversed_days(self, df_col: pd.Series, day_mask: pd.Series, 
                                        local_time: pd.Series,
                                        peak_quantile: float = 0.90,
                                        max_night_peak_ratio: float = 0.30) -> pd.Series:
        """
        ...
        for date_obj, group in grouped:
            ...
            # Handle edge cases
            if pd.isna(mean_day): mean_day = 0
            if pd.isna(mean_night): mean_night = 0
        ...
        # --- Summary ---
        print(f"    [Reversed Detection]")
        print(f"      Check 1 - Day avg * 0.8 < Night avg - 0.5: {days_failed_avg}/{total_days} days failed")
        print(f"      Check 2 - >{max_night_peak_ratio:.0%} of top {(1-peak_quantile)*100:.0f}% at solar night: {days_failed_peak}/{total_days} days failed")
        print(f"      REMOVED (either failed): {days_removed}/{total_days} days ({days_removed/max(1,total_days)*100:.1f}%)")
            
        return reversed_mask
    def _detect_variability_issues(self, series: pd.Series, day_mask: pd.Series, 
                                   window_points: int,
                                   day_cv_low: float, day_cv_high: float,
                                   day_std_low: float, day_std_high: float,
                                   night_cv_high: float, night_std_high: float,
                                   col_name: str = None) -> pd.Series:
        """
        ...
        # ====================================================================
        if not day_series.dropna().empty:
            print(f"  Checking daytime variability...")
        ...
        # ====================================================================
        if not night_series.dropna().empty:
            print(f"  Checking nighttime variability...")
    ...
    def _remove_incomplete_days(self, df: pd.DataFrame, day_mask: pd.Series,
                            completeness_threshold: float = 0.5,
                            data_columns: list = None) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        ...
        if 'solar_TIMESTAMP' in df.columns:
            ...
            print(f"  Using solar_TIMESTAMP for date grouping")
            ...
            print(f"  Using TIMESTAMP_LOCAL for date grouping (solar_TIMESTAMP not found)")
        else:
            dates = pd.Series(df.index.date, index=df.index)
            print(f"  WARNING: Using UTC dates for grouping")
        # === END FIX ===
    ...
    def _get_timewindow(self, time_window: int, timesteps_data: Union[pd.Index, pd.Series, pd.DataFrame]) -> int:
        ...
        if timestep_seconds <= 0:
            raise ValueError(f"Calculated timestep is non-positive ({timestep_seconds} seconds). "
    ...
    def plot_all_plants(self, files_to_load: Optional[list[Path]]=None, figsize=(12, 6), save_dir: str = None, skip_empty: bool = True, 
    ...
    def plot_individual_plants(self, location: str, plant_type: str, 
        ...
        plot_columns = [
            ...
            and 'Unnamed:' not in str(col) and len(col) > 0
        ]
        ...
        for column in plot_columns:
            ...
            if not valid_data_series.empty:
                ...
                ax1.plot(local_timestamps, valid_data_series.values,
                ...
                if flagged_data is not None and not flagged_data[column].dropna().empty:
                    ...
                    ax1.scatter(local_flagged_ts, flagged_points.values,
                                color='red', alpha=0.7, label='Flagged data',
                ...
                if outliers_df is not None:
                    if 'is_outlier' in outliers_df.columns:
                        ...
                        if not outlier_points.empty:
                            ...
                            ax1.scatter(local_outlier_ts, outlier_points.values,
                                    color='orange', alpha=0.7, label='Outliers',
                    ...
                    if 'is_variability_issue' in outliers_df.columns:
                        ...
                        if not variability_points.empty:
                            ...
                            ax1.scatter(local_var_ts, variability_points.values,
                                    color='purple', alpha=0.7, label='Variability issues',
                ...
                outlier_count = outliers_df['is_outlier'].sum() if outliers_df is not None and 'is_outlier' in outliers_df.columns else 0
                variability_count = outliers_df['is_variability_issue'].sum() if outliers_df is not None and 'is_variability_issue' in outliers_df.columns else 0
    ...
    def plot_histogram(self, save_dir: str = None):
        ...
        for location in self.sapf_raw_data:
            for plant_type in self.sapf_raw_data[location]:
                ...
                plot_columns = [col for col in df.columns if col != 'solar_TIMESTAMP' and col != 'TIMESTAMP' and col != 'TIMESTAMP_LOCAL' and len(col) > 0 and col not in ['lat', 'long'] and 'Unnamed:'not in str(col)]
                df = df.replace("NA", np.nan)
                for column in plot_columns:
                    try:
