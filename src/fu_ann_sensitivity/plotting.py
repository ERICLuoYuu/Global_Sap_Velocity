...
Fig 2 -> ``plot_sensitivity_heatmap``: a 2-D SWC-pct x VPD-pct heatmap of one
         sensitivity leg (diverging cmap centred at 0, '*' on significant cells).
Fig 3 -> ``plot_dual_legs``: 1-D dual-leg curves -- both sensitivities (to SWC and
         to VPD) vs SWC bin (panel a) and vs VPD bin (panel b), median + 25-75 band.
...
def plot_sensitivity_heatmap(map2d, sig_mask, title: str, out_path) -> None:
    """2-D sensitivity heatmap (Fig 2). SWC percentile on y, VPD percentile on x."""
    ...
    ax.set_ylabel("SWC percentile bin")
    ax.set_title(title)
...
def _plot_leg_panel(ax, tbl, axis_name: str) -> None:
    ...
    for leg, color, label in [("d_sm", "tab:brown", "to SWC (drying)"), ("d_vpd", "tab:blue", "to VPD (rising)")]:
...
def plot_dual_legs(by_sm_bin, by_vpd_bin, response: str, out_path) -> None:
    """Fig 3: both sensitivity legs vs SWC bin (a) and vs VPD bin (b)."""
    fig, (ax_a, ax_b) = plt.subplots(1, 2, figsize=(10, 4.2), sharey=True)
    _plot_leg_panel(ax_a, by_sm_bin, "sm_bin")
    ax_a.set_xlabel("SWC percentile bin")
    ax_a.set_ylabel(f"sensitivity of {response}")
    ax_a.set_title("(a) at each SWC bin")
    ...
    fig.suptitle(f"Disentangled SWC vs VPD sensitivity — {response}")
    fig.tight_layout()
    fig.savefig(out_path, dpi=150)
    plt.close(fig)
...
def plot_pft_panels(per_pft_by_sm_bin: dict, response: str, out_path) -> None:
    ...
    for idx, pft in enumerate(pfts):  # index-based, not zip() -> py39/Palma safe
        ...
        ax.set_title(pft)
        ax.set_xlabel("SWC percentile bin")
    for ax in flat[len(pfts) :]:
        ax.axis("off")
    if pfts:  # avoid an empty-axis legend warning when there are no PFT panels
        flat[0].set_ylabel(f"sensitivity of {response}")
        flat[0].legend(loc="best", fontsize=7)
    fig.suptitle(f"Sensitivity by PFT (at each SWC bin) — {response}")
    fig.tight_layout()
    fig.savefig(out_path, dpi=150)
    plt.close(fig)
