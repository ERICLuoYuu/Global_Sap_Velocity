...
def main(run_id="default"):
    ...
    # Save site_info_dict to config for standalone SHAP
    ...
    # =========================================================================
    # SHAP ANALYSIS (now runs as standalone script)
    # =========================================================================
    logging.info("\n" + "=" * 60)
    logging.info("=== SHAP ANALYSIS SEPARATED ===")
    logging.info("=" * 60)
    logging.info("SHAP context bundle saved. To run SHAP analysis:")
    logging.info(
        f"  python -m src.hyperparameter_optimization.run_shap_analysis_ml"
    ...
    try:
        ...
        # 2. Load SHAP importance (generated by standalone SHAP script)
        shap_csv_path = plot_dir / "shap_feature_importance.csv"
        if not shap_csv_path.exists():
            logging.warning(
                f"SHAP importance CSV not found at {shap_csv_path}. "
                "Run standalone SHAP analysis first, then re-run AOA separately."
            )
            raise FileNotFoundError(f"Run standalone SHAP first: {shap_csv_path}")
