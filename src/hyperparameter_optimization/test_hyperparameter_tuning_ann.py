...
Modified to match the CNN-LSTM implementation's windowing strategy.
"""
...
def main(run_id="default"):
    ...
    # Create windowed datasets with deterministic options (just like CNN-LSTM)
    ...
    plt.title('ANN Model: Test Predictions vs Actual')
    plt.axis('equal')
    plt.axis('square')
    plt.xlim([0, plt.xlim()[1]])
    plt.ylim([0, plt.ylim()[1]])
    plt.plot([-100, 100], [-100, 100])
    plt.savefig(plot_dir / f'ann_test_predictions_{run_id}.png')
    plt.close()
    ...
    plt.xlabel('True Values [Sap Velocity]')
    plt.ylabel('Predictions [Sap Velocity]')
    plt.title('ANN Model: Training Predictions vs Actual')
    plt.axis('equal')
    plt.axis('square')
    plt.xlim([0, plt.xlim()[1]])
    plt.ylim([0, plt.ylim()[1]])
    plt.plot([-100, 100], [-100, 100])
    plt.savefig(plot_dir / f'ann_train_predictions_{run_id}.png')
    plt.close()
    ...
    plt.ylabel('Count')
    plt.title('ANN Model: Test Error Distribution')
    plt.savefig(plot_dir / f'ann_test_error_distribution_{run_id}.png')
    plt.close()
    ...
    plt.xlabel('Prediction Error [Sap Velocity]')
    plt.ylabel('Count')
    plt.title('ANN Model: Training Error Distribution')
    plt.savefig(plot_dir / f'ann_train_error_distribution_{run_id}.png')
    plt.close()
    ...
    if hasattr(optimizer, 'history_') and optimizer.history_ is not None:
        ...
        plt.tight_layout()
        plt.savefig(plot_dir / f'ann_training_history_{run_id}.png')
        plt.close()
