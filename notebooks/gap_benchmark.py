...
def load_site_metadata(site_name: str) -> dict:
    ...
    """
    defaults = {"si_biome": "Unknown", "si_igbp": "Unknown", "si_lat": np.nan, "si_long": np.nan}
    md_file = SITE_MD_DIR / f"{site_name}_site_md.csv"
    if not md_file.exists():
        return defaults.copy()
    try:
        df_md = pd.read_csv(md_file, nrows=1)
        return {k: (df_md[k].iloc[0] if k in df_md.columns else v) for k, v in defaults.items()}
    except Exception:
        return defaults.copy()
...
def _get_plant_cols(df: pd.DataFrame) -> list:
    ...
    return [c for c in df.columns if not any(x in c.lower() for x in ["timestamp", "solar", "unnamed"])]


def _infer_site_name(stem: str) -> str:
    """Extract full site code from CSV filename stem by stripping the known suffix."""
...
def run_phase1():
                ...
                master_gaps_hourly.append(
                    {
                        ...
                        "end": str(g["end"]),
                        "duration_hours": d_h,
                        "duration_days": round(d_h / 24.0, 4),
                        "biome": biome,
                        "igbp": igbp,
                    }
                )
    ...
    df_site_summary = (
        pd.DataFrame(site_summary_rows).sort_values("completeness_pct", ascending=False).reset_index(drop=True)
    )
    ...
    _h_per = (
        df_gaps_h.groupby("site")
        .agg(n_gaps_hourly=("duration_hours", "count"), total_missing_hours=("duration_hours", "sum"))
        .reset_index()
    )
    _d_per = (
        df_gaps_d.groupby("site")
        .agg(n_gaps_daily=("duration_days", "count"), total_missing_days=("duration_days", "sum"))
        .reset_index()
    )

    _comp = _h_per.merge(_d_per, on="site", how="outer").fillna(0)
    _comp["hourly_to_daily_ratio"] = _comp["n_gaps_hourly"] / _comp["n_gaps_daily"].replace(0, np.nan)
    _comp.to_csv(STATS_OUT / "hourly_vs_daily_comparison.csv", index=False)

    print("Hourly vs Daily gap comparison (first 15 sites):")
    print(_comp[["site", "n_gaps_hourly", "n_gaps_daily", "hourly_to_daily_ratio"]].head(15).to_string(index=False))
...
def run_phase2(df_gaps_h, df_gaps_d):
    ...
    if not df_gaps_h.empty:
        df_gaps_h["size_bin"] = df_gaps_h["duration_hours"].apply(lambda x: assign_bin(x, HOURLY_BINS))
    if not df_gaps_d.empty:
        df_gaps_d["size_bin"] = df_gaps_d["duration_days"].apply(lambda x: assign_bin(x, DAILY_BINS))
    ...
    for _ax, _df, _col, _title in [
        ...
        _vals = _vals[_vals > 0]
        _log_bins = np.logspace(np.log10(max(0.5, _vals.min())), np.log10(_vals.max() + 1), 60)
        _ax.hist(_vals, bins=_log_bins, color="steelblue", edgecolor="white", alpha=0.85, linewidth=0.4)
        for _pct, _ls, _c in [(90, "--", "darkorange"), (95, ":", "crimson")]:
            _v = np.percentile(_vals, _pct)
            _ax.axvline(_v, linestyle=_ls, color=_c, linewidth=1.8, label=f"P{_pct} = {_v:.0f}")
        _ax.set_xscale("log")
    ...
    if not df_gaps_h.empty and "igbp" in df_gaps_h.columns:
        ...
        _order3 = _df3.groupby("igbp")["log_dur"].median().sort_values(ascending=False).index.tolist()
        _data3 = [_df3.loc[_df3["igbp"] == b, "log_dur"].values for b in _order3]
        _bp = _ax3.boxplot(_data3, labels=_order3, patch_artist=True, medianprops=dict(color="black", linewidth=2))
        _cmap = plt.cm.tab20
        for _i, _patch in enumerate(_bp["boxes"]):
            _patch.set_facecolor(_cmap(_i / max(len(_order3), 1)))
            _patch.set_alpha(0.75)
    else:
        _ax3.text(0.5, 0.5, "No PFT (IGBP) data available", ha="center", va="center", transform=_ax3.transAxes)
    ...
    if not df_gaps_h.empty and df_gaps_h["start"].notna().any():
        _top20 = (
            df_gaps_h.groupby("site")["duration_hours"].count().sort_values(ascending=False).head(20).index.tolist()
        )
        ...
        _agg = _df4.groupby(["site", "month_period"])["duration_hours"].sum().reset_index()
        ...
        _pivot_df = _agg.pivot(index="site", columns="month_period", values="frac").reindex(_top20).fillna(0)
        _pivot_df.columns = [str(c) for c in _pivot_df.columns]
        _im = _ax4.imshow(_pivot_df.values, aspect="auto", cmap="YlOrRd", vmin=0, vmax=1, interpolation="nearest")
        ...
        _ax4.set_xticklabels([_pivot_df.columns[i] for i in _ticks], rotation=45, ha="right", fontsize=7)
        _ax4.set_yticks(range(len(_top20)))
        _ax4.set_yticklabels(_top20, fontsize=8)
    else:
        _ax4.text(0.5, 0.5, "No gap data available", ha="center", va="center", transform=_ax4.transAxes)
    ...
    for _ax, _df, _bins, _title, _c in [
        ...
        _bars = _ax.bar(_bkeys, _counts, color=_c, edgecolor="white", linewidth=0.5, alpha=0.85)
...
def run_phase3(df_site_summary):
    ...
    # Deduplicate by site name to prevent repeated processing
    selected_sites = (
    ...
    for _, _row in selected_sites.iterrows():
        ...
        for _col_h, _seg_h in _hourly_segments:
            ...
            if _col_h in _df_d.columns:
                _daily_segments = get_all_qualifying_segments(_df_d[[_col_h]], min_len=30)
            else:
                _daily_segments = []
    ...
    if ground_truth_store:
        ...
        if _tr:
            print(
                f"\nVerification: 24h gap injection → {_ts.notna().sum()} → {_tr[0][0].notna().sum()} non-null values"
            )
        else:
...
def run_phase6(df_results, df_agg):
    ...
    for _scale in ["hourly", "daily"]:
        ...
        for _ai, _metric in enumerate(_metrics4):
            ...
            if _metric == "r2":
                _ax.axhline(R2_THRESHOLD, color="red", linestyle="--", linewidth=1, label=f"R²={R2_THRESHOLD}")
            if _metric == "nse":
                _ax.axhline(NSE_THRESHOLD, color="orange", linestyle="--", linewidth=1, label=f"NSE={NSE_THRESHOLD}")
            _ax.set_xscale("log")
            ...
            _ax.set_xticklabels([_human_size(g, _scale) for g in _gsizes], rotation=40, ha="right", fontsize=7)

        _handles = [
            Patch(facecolor=GROUP_COLORS[g], label=f"Group {g}")
            for g in ["A", "B", "C", "D", "Ce", "De"]
            if g in GROUP_COLORS
        ]
        _fig.legend(handles=_handles, loc="lower center", ncol=6, fontsize=9, bbox_to_anchor=(0.5, -0.02))
        _fig.suptitle(f"Gap-Filling Performance vs Gap Size ({_scale.capitalize()})", fontweight="bold", fontsize=13)
    ...
    for _scale in ["hourly", "daily"]:
        ...
        _fig2, _ax2 = plt.subplots(figsize=(max(10, len(_gsizes) * 1.1), max(6, len(_m_order) * 0.55)))
        sns.heatmap(
            _pivot,
            cmap="RdYlGn",
            vmin=0,
        ...
        _ax2.set_xticklabels([_human_size(float(c), _scale) for c in _gsizes], rotation=40, ha="right", fontsize=8)
        ...
        if _thr is not None and _thr in _gsizes:
            ...
            _ax2.axvline(_tx, color="red", linewidth=2, linestyle="--", label=f"Threshold: {_human_size(_thr, _scale)}")
    ...
    def _find_crossover(x1: np.ndarray, y1: np.ndarray, x2: np.ndarray, y2: np.ndarray) -> list:
    ...
    for _scale in ["hourly", "daily"]:
        ...
        for _i in range(len(_grp_order) - 1):
            ...
            for _cx in _find_crossover(_x1, _y1, _x2, _y2):
                ...
                _ax3.annotate(
                    ...
                    xy=(_cx, _cy),
                    xytext=(_cx * 1.2, _cy + 0.05),
                    arrowprops={"arrowstyle": "->", "color": "grey"},
                    fontsize=7,
                    color="grey",
                )

        _ax3.axhline(R2_THRESHOLD, color="red", linestyle="--", linewidth=1, label=f"R²={R2_THRESHOLD}")
        _ax3.set_xscale("log")
        ...
        _ax3.set_xticklabels([_human_size(g, _scale) for g in _gsizes], rotation=40, ha="right", fontsize=8)
    ...
    for _scale in ["hourly", "daily"]:
        ...
        for _r in _rows_dm:
            print(
                f"{_r['Gap Size Range']:>12}  {_r['Best Method']:>18}  "
        ...
        if _rows_dm:
            ...
            _fig4, _ax4 = plt.subplots(figsize=(13, max(3, len(_rows_dm) * 0.55 + 1.5)))
            _ax4.axis("off")
            _tbl = _ax4.table(cellText=_cell_data, colLabels=_col_labels, loc="center", cellLoc="center")
            ...
            _ax4.set_title(f"Decision Matrix — {_scale.capitalize()} Scale", fontweight="bold", pad=12)
            _fig4.tight_layout()
    ...
    for _scale in ["hourly", "daily"]:
        ...
        _ax5.axhline(R2_THRESHOLD, color="red", linestyle="--", linewidth=1.5, label=f"R² = {R2_THRESHOLD} (threshold)")
        _ax5.axhline(NSE_THRESHOLD, color="orange", linestyle=":", linewidth=1.2, label=f"NSE = {NSE_THRESHOLD}")
        if _thr is not None:
            _ax5.axvline(_thr, color="red", linewidth=1.5, linestyle=":")
            _ymax = float(max(_y)) if len(_y) else 1.0
            _ax5.fill_betweenx([0, _ymax], _thr, _x.max() * 2, color="red", alpha=0.08, label="Do-not-fill zone")
            _ax5.annotate(
        ...
        _ax5.set_ylabel("R²")
        _ax5.set_title(f"Reliability Threshold — {_scale.capitalize()} Scale", fontweight="bold")
        _ax5.set_xticks(_gsizes)
        _ax5.set_xticklabels([_human_size(g, _scale) for g in _gsizes], rotation=40, ha="right", fontsize=8)
        _ax5.legend()
    ...
    if not _h_best.empty and not _d_best.empty:
        ...
        _ax7.plot(
            _d_best.index.values.astype(float),
            _d_best.values,
            color="#4CAF50",
            linewidth=2.5,
            marker="s",
        ...
        _ax7.axhline(R2_THRESHOLD, color="red", linestyle="--", linewidth=1.2, alpha=0.7, label=f"R²={R2_THRESHOLD}")
        _ax7.set_xscale("log")
        _ax7.set_xlabel("Gap size (days, log scale)")
        _ax7.set_ylabel("R² (best method)")
        _ax7.set_title("Hourly vs Daily Scale: Gap-Filling Performance", fontweight="bold")
        _ax7.legend()
        _fig7.tight_layout()
    ...
    print("=" * 60)
    if threshold_hourly:
        print(f"  Hourly: do NOT fill gaps > {threshold_hourly}h ({threshold_hourly / 24:.1f} days)")
    else:
