...
"""
Enhanced Environmental Data Extraction Tool with Google Earth Engine Support
...
3. Plant functional types (MCD12Q1 v061, median from 2001-2022) using Google Earth Engine
4. Canopy height
"""
...
#==============================
# Plant Functional Type Data - Using Google Earth Engine
#==============================
...
# NASA Earth Data Authentication - only needed for direct downloads, not for GEE
...
# MODIS MCD12Q1 data URLs - only needed for direct downloads, not for GEE
...
def download_and_process_modis_pft_gee(sites_df, output_dir):
    """
    Use Google Earth Engine to extract MODIS MCD12Q1 PFT data for multiple years
    ...
    """
    try:
        # Try to import necessary GEE libraries
        import ee
        import time
        ...
        try:
            ...
            try:
                ee.Authenticate()
                ee.Initialize()
                logger.info("Successfully authenticated and initialized Google Earth Engine")
            except Exception as auth_e:
                logger.error(f"Could not authenticate with Google Earth Engine: {auth_e}")
                logger.error("Falling back to direct file download method")
                return download_and_process_modis_pft(sites_df, output_dir)
        ...
        # Define the MCD12Q1 collection with PFT classification (Type 5)
        mcd12q1_collection = ee.ImageCollection('MODIS/006/MCD12Q1')
        
        # Process each year
        for year in range(START_YEAR, END_YEAR + 1):
            logger.info(f"Processing MCD12Q1 PFT data for year {year} using Google Earth Engine")
            
            # Filter the collection for the specific year
            year_start = ee.Date.fromYMD(year, 1, 1)
            year_end = ee.Date.fromYMD(year + 1, 1, 1)
            year_collection = mcd12q1_collection.filterDate(year_start, year_end)
            
            # In the download_and_process_modis_pft_gee function:
            # After the line:
            year_image = ee.Image(year_collection.first())

            # Add these checks:
            try:
                # Try to get band names to check if we have a valid image
                band_names = year_image.bandNames().getInfo()
                if band_names is None:
                    logger.warning(f"No MODIS MCD12Q1 image available for year {year}")
                    continue
                    
                # Only proceed if we have a valid image
                pft_image = year_image.select('LC_Type5')
                
            except Exception as e:
                logger.warning(f"Error accessing MODIS data for year {year}: {e}")
                continue
            
            # Extract values for each site in batches to avoid rate limits
            batch_size = 50  # Process 50 sites at a time
            num_batches = (len(sites_df) + batch_size - 1) // batch_size
            
            for batch_idx in tqdm(range(num_batches), desc=f"Year {year} PFT extraction"):
                start_idx = batch_idx * batch_size
                end_idx = min(start_idx + batch_size, len(sites_df))
                batch_df = sites_df.iloc[start_idx:end_idx]
                
                # Create a list of points
                points = [ee.Geometry.Point(lon, lat) for lat, lon in zip(batch_df['lat'], batch_df['lon'])]
                
                # Extract values for all points in the batch
                if points:
                    try:
                        # Create a feature collection from the points
                        fc = ee.FeatureCollection([ee.Feature(point) for point in points])
                        
                        # Extract values
                        pft_values = pft_image.reduceRegions(
                            collection=fc,
                            reducer=ee.Reducer.first(),
                            scale=500  # MCD12Q1 is 500m resolution
                        ).getInfo()
                        
                        # Process the extracted values
                        for i, feature in enumerate(pft_values['features']):
                            site_idx = batch_df.index[i]
                            if 'properties' in feature and 'first' in feature['properties']:
                                pft_value = feature['properties']['first']
                                if pft_value is not None:
                                    # Store in DataFrame
                                    sites_df.at[site_idx, f'pft_{year}'] = pft_value
                                    # Add to time series
                                    site_pfts[site_idx].append(pft_value)
                    except Exception as batch_error:
                        logger.error(f"Error processing batch {batch_idx} for year {year}: {batch_error}")
        ...
        logger.info(f"Successfully extracted median PFT values for {valid_count} out of {len(sites_df)} sites using Google Earth Engine")
        
        return sites_df
    
    except ImportError:
        logger.warning("Google Earth Engine Python API (ee) not available")
        logger.warning("Falling back to direct file download method")
        return download_and_process_modis_pft(sites_df, output_dir)
    except Exception as e:
        logger.error(f"Error using Google Earth Engine for PFT extraction: {e}")
        logger.warning("Falling back to direct file download method")
        return download_and_process_modis_pft(sites_df, output_dir)

# Original MODIS functions as fallback if GEE is not available
...
def find_mcd12q1_files(base_url, year, session):
    ...
    try:
        ...
        logger.debug(f"First 500 chars of response from {year_url}: {response.text[:500]}")
        
        # Parse the HTML to find HDF files
        hdf_files = []
        # Updated pattern to match actual file naming on the server
...
def extract_modis_pft(hdf_file, lat, lon):
    ...
    """
    try:
        # First list all available subdatasets to find the right one
        with rasterio.open(hdf_file) as src:
            subdatasets = src.subdatasets
            
            # Look for the PFT layer (LC_Type5)
            pft_layer = None
            for subdataset in subdatasets:
                if 'LC_Type5' in subdataset:
                    pft_layer = subdataset
                    break
            
            if not pft_layer:
                logger.warning(f"Could not find LC_Type5 layer in {os.path.basename(hdf_file)}")
                return None
        
        # Now open the specific PFT layer
        with rasterio.open(pft_layer) as src:
            ...
            if existing_files:
                ...
                for date in dates:
                    ...
                    try:
                        ...
                        # Find the specific tile we need with updated pattern
...
def extract_environmental_data(csv_path, output_dir="env_data"):
    ...
    #--------------------------------------------------------------------------
    # Step 3: Plant Functional Types - ENHANCED WITH GEE
    #--------------------------------------------------------------------------
    current_step += 1
    logger.info(f"Step {current_step}/{total_steps}: Processing plant functional types using Google Earth Engine")
    
    # Use GEE for MODIS MCD12Q1 PFT (2001-2022 median)
    logger.info("Processing MCD12Q1 PFT data (median from 2001-2022) using Google Earth Engine")
    
    try:
        # Process using the GEE-based function
        df_with_pft = download_and_process_modis_pft_gee(df, output_dir)
        if df_with_pft is not None:
            df = df_with_pft
            logger.info("Successfully processed MODIS MCD12Q1 PFT data with Google Earth Engine")
        else:
            # Fall back to placeholder approach
            logger.warning("GEE extraction failed, falling back to placeholder")
            pft_file = create_pft_placeholder(output_dir)
            
            # Process with extract_raster_value
            if not pft_file:
                logger.error("Failed to create PFT placeholder data")
            else:
                # Add columns to dataframe
                ...
                # Extract values for each site
                logger.info(f"Extracting plant functional types from placeholder...")
        ...
        logger.error(f"Error in GEE PFT processing: {e}")
        # Fall back to original placeholder method
        pft_file = create_pft_placeholder(output_dir)
    ...
    with open(metadata_path, "w", encoding='utf-8') as f:
        ...
        f.write(f"     - Extracted using Google Earth Engine\n")
