...
- Top: Sap flow data (with Outliers, Variability Flags, Gap-Filled)
...
class FastSapFlowPlotterDual:
    ...
    def __init__(
        ...
        output_dir: str,
        env_dir: str,
        outlier_dir: str = None,
        variability_dir: str = None,
        flagged_dir: str = None,
        reversed_dir: str = None,
        gap_filled_dir: str = None,
        env_var: str = "sw_in",
    ):
        """
    ...
    def _find_matching_env_file(self, location: str, plant_type: str) -> Path | None:
    ...
    def _load_env_data(self, location: str, plant_type: str) -> pd.DataFrame | None:
    ...
    def _load_gap_filled_data(self, sap_df: pd.DataFrame, column: str):
        """Load per-column gap-fill mask file.

        Reads from gap_filled_dir/{column}_gap_filled.csv which has columns:
        timestamp, solar_TIMESTAMP, value, is_gap_filled
        Returns only the rows where is_gap_filled is True.
        """
        if self.gap_filled_dir is None:
            return None

        gf_path = self.gap_filled_dir / f"{column}_gap_filled.csv"
        if not gf_path.exists():
            return None

        try:
            gf_df = pd.read_csv(gf_path, parse_dates=["solar_TIMESTAMP"])
            if "is_gap_filled" not in gf_df.columns:
                return None
            filled_points = gf_df[gf_df["is_gap_filled"] == True]
            if filled_points.empty:
                return None
            return filled_points[["solar_TIMESTAMP", "value"]].copy()
        except Exception as e:
            print(f"    WARNING: could not load gap-filled data for {column}: {e}")
            return None

    def create_dual_subplot(
        self, sap_df: pd.DataFrame, env_df: pd.DataFrame | None, column: str, location: str, plant_type: str
    ):
        """
        ...
        # 6. Gap-Filled Data (Green circle-open)
        gap_filled_df = self._load_gap_filled_data(sap_df, column)
        gap_filled_count = 0
        if gap_filled_df is not None and not gap_filled_df.empty:
            gap_filled_count = len(gap_filled_df)
            fig.add_trace(
                go.Scatter(
                    x=gap_filled_df["solar_TIMESTAMP"],
                    y=gap_filled_df["value"],
                    mode="markers",
                    name="Gap Filled",
                    marker=dict(
                        size=5,
                    ...
                    hovertemplate="<b>Time:</b> %{x}<br><b>Value:</b> %{y:.3f}<br><b>Type:</b> Gap Filled<extra></extra>",
                    legendgroup="sap",
                ),
                row=1,
                col=1,
