...
UNIT_PATTERNS = {
    ...
    r"cm[3]\s*[/\s]*cm[2]\s*[\*\s]*(10\s*min|20.*min|30.*min|min|h|hour|s|sec)": {
        ...
        "time_patterns": {
            "10": 6.0,  # 10 min -> detected as sub-hourly, summed to hourly
            "20": 3.0,  # 20 min -> detected as sub-hourly, summed to hourly
            "30": 2.0,  # 30 min -> detected as sub-hourly, summed to hourly
...
def parse_unit_string(unit_str: str) -> tuple[float, str, str, int | None]:
    """
    Parse a unit string and return conversion factor, target unit, description,
    and sub-hourly interval in minutes.

    For sub-hourly accumulated flux (e.g. cm3 cm-2 20min-1), the factor covers
    only spatial conversion (e.g. mm->cm). The caller should resample to hourly
    using sum() instead of multiplying by a time factor.

    Returns:
        (conversion_factor, target_unit, description, interval_minutes)
        interval_minutes: None for hourly/rate units, int for sub-hourly accumulated flux
    """
    if pd.isna(unit_str) or not unit_str:
        return 1.0, "unknown", "Unknown unit", None
            ...
            if "time_patterns" in info:
                ...
                for time_key, time_mult in info["time_patterns"].items():
                    if time_key in unit_str_clean:
                        if time_key in ("10", "20", "30"):
                            # Sub-hourly accumulated flux — will be summed to hourly
                            interval_minutes = int(time_key)
                        else:
                            # Rate conversion (per second, per minute, etc.)
                ...
                return factor, target, info["description"], interval_minutes

            target = TARGET_SAPWOOD_UNIT if info["type"] == "sapwood" else TARGET_PLANT_UNIT
            return factor, target, info["description"], None

    # Special case handling for specific known formats
    if "cm/hour" in unit_str_clean or "cm/h" in unit_str_clean:
        return 1.0, TARGET_SAPWOOD_UNIT, "cm/hour velocity", None
    ...
    if "cm3/cm2" in unit_str_clean or "cm3 cm" in unit_str_clean or "cm3cm" in unit_str_clean:
        # Check time unit — sub-hourly accumulated flux returns interval_minutes
        if "10min" in unit_str_clean or "10 min" in unit_str_clean:
            return 1.0, TARGET_SAPWOOD_UNIT, "cm3 cm-2 10min-1 -> hourly sum", 10
        elif "20" in unit_str_clean and "min" in unit_str_clean:
            return 1.0, TARGET_SAPWOOD_UNIT, "cm3 cm-2 20min-1 -> hourly sum", 20
        elif "30" in unit_str_clean and "min" in unit_str_clean:
            return 1.0, TARGET_SAPWOOD_UNIT, "cm3 cm-2 30min-1 -> hourly sum", 30
        else:
            return 1.0, TARGET_SAPWOOD_UNIT, "cm3 cm-2 h-1", None

    if "kg" in unit_str_clean and "cm" in unit_str_clean and "2" in unit_str_clean:
        # kg per area per time
        return 1000.0, TARGET_SAPWOOD_UNIT, "kg cm-2 time-1", None
    ...
    if "mm3" in unit_str_clean or "mm-2" in unit_str_clean:
        ...
        if "s-1" in unit_str_clean or unit_str_clean.endswith("s"):
            return 360.0, TARGET_SAPWOOD_UNIT, "mm3 mm-2 s-1 -> cm3 cm-2 h-1 (x360)", None
        return 0.1, TARGET_SAPWOOD_UNIT, "mm3 mm-2 -> cm3 cm-2 (x0.1)", None

    print(f"    WARNING: Unknown unit format: '{unit_str}' - using factor=1.0")
    return 1.0, "unknown", f"Unknown: {unit_str}", None


def convert_sapflow_data(
    sapflow_df: pd.DataFrame, plant_md_df: pd.DataFrame, target_type: str = "sapwood", site_code: str = None
    ...
    factor, detected_target, desc, interval_minutes = parse_unit_string(orig_unit)
    ...
    # Before 2024-03-20: 20 min, From 2024-03-20: 30 min
    # Resample to hourly using sum (not multiply by factor)
    if site_code and "DE-Har" in site_code:
        print("    Special handling for DE-Har: mixed 20min/30min -> hourly sum")
        ...
        # Apply spatial factor if needed (e.g., mm->cm), but NOT time factor
        if factor != 1.0:
            for col in data_cols:
                converted_df[col] = converted_df[col] * factor
            print(f"    Applied spatial conversion factor: {factor}")

        mask_before = converted_df.index < cutoff_date
        mask_after = converted_df.index >= cutoff_date

        df_before = converted_df.loc[mask_before, data_cols]
        df_after = converted_df.loc[mask_after, data_cols]

        print(f"    Before 2024-03-20 (20min): {len(df_before)} rows -> hourly sum")
        print(f"    From 2024-03-20 (30min): {len(df_after)} rows -> hourly sum")

        # Sum sub-hourly values to hourly (min_count=1: NaN if all values are NaN)
        parts = []
        for part_df, label in [(df_before, "20min"), (df_after, "30min")]:
            if len(part_df) > 0:
                hourly = part_df.resample("h").sum(min_count=1)
                parts.append(hourly)

        if parts:
            converted_df = pd.concat(parts).sort_index()
            ...
            conversion_log.append(
                {
                    "pl_code": col,
                    "orig_unit": orig_unit,
                    "factor": "hourly_sum (20min/30min)",
                    "target_unit": target_unit,
                    "description": "DE-Har: sum sub-hourly values per hour",
                }
            )

        updated_md["pl_sap_units"] = target_unit
        updated_md["conversion_factor"] = "hourly_sum (20min/30min)"

        return converted_df, updated_md, conversion_log

    # Sub-hourly accumulated flux: resample to hourly using sum
    if interval_minutes is not None:
        expected_per_hour = 60 // interval_minutes
        print(f"    Sub-hourly data ({interval_minutes}min, {expected_per_hour}/hour): resampling to hourly using sum")
        ...
        # Sum sub-hourly values to hourly (min_count=1: NaN if all values are NaN)
        converted_df = converted_df[data_cols].resample("h").sum(min_count=1)
            ...
            conversion_log.append(
                {
                    "pl_code": col,
                    "orig_unit": orig_unit,
                    "factor": f"hourly_sum({interval_minutes}min)",
                    "target_unit": target_unit,
                    "description": f"Sum {expected_per_hour} x {interval_minutes}min per hour",
                }
            )

        updated_md["pl_sap_units"] = target_unit
        updated_md["conversion_factor"] = f"hourly_sum({interval_minutes}min)"

        return converted_df, updated_md, conversion_log

    # Standard conversion for hourly/rate data (multiply by factor)
...
def process_site_folder(site_folder: Path, output_folder: Path):
    ...
    for sapflow_file in sapflow_files:
        ...
        print(
            f"    Original units: {[str(u).encode('ascii', 'replace').decode() for u in plant_md_df['pl_sap_units_orig'].unique()]}"
        )
        ...
        converted_df, updated_md, log = convert_sapflow_data(sapflow_df, plant_md_df, target_type, site_code=site_code)
...
def main():
    ...
    if all_summaries:
        ...
        summary_df["orig_units"] = summary_df["orig_units"].apply(lambda x: str(x).encode("ascii", "replace").decode())
