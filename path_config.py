...
class PathConfig:
    ...
    def __init__(
        ...
        # Era5 extracted data
        self.era5_discrete_data_path = (
            self.extracted_data_dir / "era5land_site_data" / self.scale / "era5_extracted_data.csv"
        )
        # LAI extracted data
        self.globmap_lai_data_path = (
            self.extracted_data_dir / "globmap_lai_site_data" / self.scale / "extracted_globmap_lai_hourly.csv"
        )
        self.pft_data_path = self.extracted_data_dir / "landcover_data" / self.scale / "landcover_output.csv"

        # LAI extracted data
        self.lai_extracted_data_path = (
            self.extracted_data_dir / "globmap_lai_site_data" / self.scale / "extracted_globmap_lai.csv"
        )
        # ENV extracted data
        self.env_extracted_data_dir = self.extracted_data_dir / "env_site_data" / self.scale
        self.env_extracted_data_path = (
            self.extracted_data_dir / "env_site_data" / self.scale / "site_info_with_env_data.csv"
        )

        self.terrain_attributes_data_path = (
    ...
    def save_config(self, filepath: Path | None = None):
        """
...
def get_development_paths() -> PathConfig:
    ...
    return PathConfig(scale="plant", base_data_dir="./data", base_output_dir="./dev_outputs")


def get_production_paths() -> PathConfig:
    """Get paths configured for production environment."""
    return PathConfig(scale="plant", base_data_dir="/data/production", base_output_dir="/outputs/production")


def get_testing_paths() -> PathConfig:
    """Get paths configured for testing environment."""
    return PathConfig(scale="plant", base_data_dir="./tests/data", base_output_dir="./tests/outputs")
