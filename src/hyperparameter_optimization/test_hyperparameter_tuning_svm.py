...
def main():
    ...
    # Convert TensorFlow datasets to numpy arrays for svr
    def convert_tf_dataset_to_xy(dataset):
        ...
        # Reshape for svr (flatten window dimension)
    ...
    # Using the function to prepare data for svr
    ...
    param_grid = {
            ...
            'degree': [3, 4],  # Polynomial degree
            'coef0': [1,],  # Independent term in kernel
            'epsilon': [0.5]
        }
    ...
    optimizer = MLOptimizer( param_grid=param_grid, scoring='r2', model_type='svm', task='regression', random_state=42)
    ...
    plt.title('svr Model: Test Predictions vs Actual')
    plt.savefig(plot_dir / 'svr_test_predictions.png')
    plt.close()
    ...
    plt.savefig(plot_dir / 'svr_train_predictions.png')
    plt.close()
    ...
    plt.ylabel('Count')
    plt.title('svr Model: Test Error Distribution')
    plt.savefig(plot_dir / 'svr_test_error_distribution.png')
    plt.close()
    ...
    plt.xlabel('Prediction Error [Sap Velocity]')
    plt.ylabel('Count')
    plt.title('svr Model: Training Error Distribution')
    plt.savefig(plot_dir / 'svr_train_error_distribution.png')
    plt.close()
