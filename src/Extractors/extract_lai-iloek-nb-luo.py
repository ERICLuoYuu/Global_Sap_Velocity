...
def extract_modis_data(point, start_date, end_date):
    ...
    region = point.buffer(4500).bounds() # 4.5km buffer = 9km box
    ...
    # Check if 'Lai' column exists before filtering
    if 'Lai' not in df.columns:
        print("Warning: No 'Lai' column in MODIS data. Returning empty DataFrame.")
        return pd.DataFrame()
    if df.dropna().empty:
...
def extract_avhrr_sr_data(point, start_date, end_date):
    ...
    try:
        ...
        raise NotImplementedError("Direct AVHRR SR extraction not implemented.")
...
def extract_avhrr_from_ndvi(point, start_date, end_date):
    ...
    try:
        ...
        avhrr_collection = ee.ImageCollection('NASA/GIMMS/3GV0')
        
        filtered = avhrr_collection.filterDate(start_date, end_date).select('ndvi')
        raw_data = filtered.getRegion(point, scale=9277).getInfo()
...
class PixelSRLAIRelationship:
    ...
    def build_relationship(self, modis_lai_df, avhrr_sr_df, site_info=None):
        ...
        """
         # --- START DEBUGGING ---
        print("--- Debugging: Inside build_relationship ---")
        print("1. Inspecting incoming MODIS LAI data:")
        print(modis_lai_df.describe())
        if modis_lai_df['Lai'].max() > 15:
            print("\n*** WARNING: MODIS LAI values seem too high! Check scaling. ***\n")

        print("\n2. Inspecting incoming AVHRR SR data:")
        print(avhrr_sr_df.describe())
        # --- END DEBUGGING ---
        ...
        modis_resampled = modis_lai_df.resample('16D').mean()
        avhrr_resampled = avhrr_sr_df.resample('16D').mean()
...
if __name__ == "__main__":
    ...
    if not lai_high_freq_hourly.empty:
        ...
        lai_high_freq_hourly.to_csv('enhanced_lai_high_freq_hourly.csv', index=False)
    ...
    if not lai_low_freq_hourly.empty:
        ...
        lai_low_freq_hourly.to_csv('enhanced_lai_low_freq_hourly.csv', index=False)
