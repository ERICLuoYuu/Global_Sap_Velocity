...
from scipy.special import inv_boxcox  # noqa: E402
from scipy.stats import boxcox  # noqa: E402
...
def group_pft_for_summary_plots(shap_values, X_df, feature_names, pft_cols):
    ...
    if not pft_indices:
        logging.debug("No PFT columns found to group.")
        return shap_values, X_df, feature_names

    logging.debug(f"Grouping {len(pft_indices)} PFT features...")
...
def plot_pft_contribution_comparison(
    ...
    for i, pft in enumerate(unique_pfts):
        ...
        ax.bar(x + offset, values, width, label=pft, color=PFT_COLORS.get(pft, "gray"), alpha=0.8, edgecolor="black")
...
def plot_seasonal_drivers_by_hemisphere(
    ...
    for ax, (hemi_name, hemi_df) in zip(axes, hemisphere_data.items(), strict=False):
        ...
        color_map = dict(zip(plot_features, colors[: len(plot_features)], strict=False))
...
def run_shap_analysis(
    ...
    if X_scaled.shape[1] != len(feature_names):
        raise ValueError(
            f"Feature count mismatch: X_scaled has {X_scaled.shape[1]} features, "
            f"but {len(feature_names)} names provided"
        )
    if X_scaled.shape[1] != X_original.shape[1]:
        raise ValueError(
            f"Shape mismatch: X_scaled has {X_scaled.shape[1]} features, but X_original has {X_original.shape[1]}"
        )
    ...
    plt.close()
    logging.info("  Saved: shap_summary_beeswarm.png")
    ...
    plt.close()
    logging.info("  Saved: shap_global_importance_bar.png")
    ...
    for i, feature in enumerate(top_features):
        ...
        try:
            sns.regplot(
                x=x_valid, y=y_valid, scatter=False, lowess=True, ax=ax, color="blue", line_kws={"linewidth": 2}
            )
    ...
    plt.close()
    logging.info("  Saved: shap_partial_dependence.png")
    ...
    for i, feature in enumerate(top_4_features):
        ...
        plt.colorbar(scatter, ax=ax, shrink=0.7, label="SHAP Value")
        ax.set_title(f"Spatial Contribution: {feature}", fontsize=13, fontweight="bold")

        # Set extent based on data
        ax.set_extent(
            [lon_valid.min() - 5, lon_valid.max() + 5, lat_valid.min() - 5, lat_valid.max() + 5], crs=ccrs.PlateCarree()
        )
    ...
    for idx, name in [(high_idx, "High_Flow"), (low_idx, "Low_Flow")]:
        ...
        plt.title(f"Why did the model predict {name}?\nSite: {site_bg[idx]}", fontsize=12, fontweight="bold")
        plt.tight_layout()
...
# Inline helper functions deleted — now imported from shared modules above.
# Deleted: add_sap_flow_features, calculate_soil_hydraulics_sr2006,
#   apply_feature_engineering, setup_logging, add_time_features,
#   create_windows_from_segments, get_predictions, convert_windows_to_numpy,
#   create_spatial_groups
...
def main(run_id="default"):
    ...
    logging.debug(f"Data files: {data_list}")
    ...
    for data_file in data_list:
        try:
            ...
            if requested_pft and pft_col and pft_col in df.columns:
                pft_cat = pd.Categorical(df[pft_col], categories=all_possible_pft_types)
            ...
            # Plant-level columns are optional — don't skip sites missing them
            _optional_cols = {
                "pl_dbh",
                "pl_sens_meth",
                "pl_species",
            ...
            _required_missing = [c for c in missing_cols if c not in _optional_cols]
            if _required_missing:
                logging.warning(
                    f"Warning: Missing required columns: {_required_missing} in {data_file.name}: skipping."
                )
                continue
            # Add missing optional cols as NaN so downstream code works
            for _oc in missing_cols:
                if _oc in _optional_cols:
                    df[_oc] = np.nan
            ...
            # Drop text columns used for one-hot encoding (already encoded as numeric features)
            for _text_col in ["pl_sens_meth", "pl_species", "pl_code"]:
                if _text_col in df.columns:
                    df = df.drop(columns=[_text_col])
    ...
    try:
        ...
        with open(units_path, "w") as f:
            json.dump(
                {"shap_units": SHAP_UNITS, "feature_units": feature_units_used, "all_feature_units": FEATURE_UNITS},
                f,
                indent=2,
            )
        logging.info(f"    Feature units saved to: {units_path}")
