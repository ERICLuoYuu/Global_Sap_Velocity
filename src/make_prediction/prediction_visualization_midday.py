...
# =============================================================================
# Helper Function for Block Size Calculation
# =============================================================================
...
def plot_sap_velocity_map(grid_data, extent, output_img='sap_velocity_map.png', max_size=5000,
                         shapefile_path='./data/raw/grided/tree_cover_shapefile_dissolved/tree_cover_shapefile_dissolved.shp', 
                         world_shapefile_path='./data/raw/world/world-administrative-boundaries.shp',
                         percentile_range=(10, 90), cmap='viridis', world_map_color='lightgray', 
                         world_map_linewidth=0.5):
    """
    Visualize the sap velocity raster with proper geographical coordinates and a world map base layer.
    ...
    world_shapefile_path : str, optional
        Path to a world shapefile to use as base map (e.g., Natural Earth data)
    percentile_range : tuple
        (min_percentile, max_percentile) for color scaling, to control contrast
    cmap : str
        Name of the colormap to use
    world_map_color : str
        Color for the world map features
    world_map_linewidth : float
        Line width for the world map features
    """
    import os
    import numpy as np
    import matplotlib.pyplot as plt
    ...
    # Load and plot world map as base layer
    if world_shapefile_path and os.path.exists(world_shapefile_path):
        try:
            import geopandas as gpd
            import cartopy.crs as ccrs
            from shapely.geometry import box
            
            print(f"Loading world shapefile: {world_shapefile_path}")
            world = gpd.read_file(world_shapefile_path)
            
            # Define the bounding box for our area of interest
            bbox = box(min_lon, min_lat, max_lon, max_lat)
            
            # Clip the world shapefile to our extent with a buffer to ensure coverage
            buffer_degrees = max((max_lon - min_lon), (max_lat - min_lat)) * 0.1  # 10% buffer
            buffer_bbox = box(
            ...
            # Only process countries that intersect with our buffered extent
            countries_to_plot = world[world.geometry.intersects(buffer_bbox)]
            
            # Plot the world map
            countries_to_plot.boundary.plot(
                ax=ax, 
                edgecolor=world_map_color, 
                linewidth=world_map_linewidth, 
                zorder=1  # Ensure it's below the data
            )
            print(f"World map base layer added")
            
        except ImportError as ie:
            print(f"Warning: Required library not found for world map: {ie}")
            print("Install geopandas and cartopy for world map support")
        except Exception as world_err:
            print(f"Warning: Failed to add world map: {world_err}")
    ...
    # Display image with transparency to allow base map to show through
    img = ax.imshow(
        masked_plot_data, 
        cmap=cmap, 
        origin='upper', 
        ...
        alpha=0.7,  # Add some transparency to see the base map
        zorder=2    # Ensure it's above the world map
    )
...
# =============================================================================
# Mid-day Filtering Helper Function (NEW)
# =============================================================================
def filter_for_midday(ddf, sw_in_threshold=15, min_solar_hour=10, max_solar_hour=14):
    """
    Filter DataFrame for mid-day values based on local solar time (approximated from longitude).
    
    Args:
        ddf (dask.dataframe): Input Dask DataFrame
        sw_in_threshold (float): Minimum solar radiation threshold
        min_solar_hour (int): Start of mid-day window (local solar time)
        max_solar_hour (int): End of mid-day window (local solar time)
        
    Returns:
        dask.dataframe: Filtered DataFrame containing only mid-day measurements
    """
    print(f"Filtering for mid-day values (local solar time between {min_solar_hour}:00-{max_solar_hour}:00)")
    
    # Check if timestamp/date columns exist
    timestamp_cols = ['timestamp.1']
    
    if timestamp_cols:
        time_col = timestamp_cols[0]  # Use the first discovered time column
        print(f"Using time column: '{time_col}'")
        
        try:
            # Extract the hour from the timestamp (methods depend on column type)
            # First attempt to parse as datetime if not already
            if ddf[time_col].dtype != 'datetime64[ns]':
                print(f"Converting '{time_col}' to datetime...")
                ddf['hour_utc'] = dd.to_datetime(ddf[time_col]).dt.hour
            else:
                ddf['hour_utc'] = ddf[time_col].dt.hour
                
            # Calculate approximate local solar hour from UTC and longitude
            # Each 15° longitude = 1 hour difference
            print("Calculating local solar hours from longitude...")
            ddf['local_solar_hour'] = (ddf['hour_utc'] + (ddf['longitude'] / 15)) % 24
            
            # Filter for mid-day hours and minimum solar radiation
            print(f"Filtering for local hours {min_solar_hour}-{max_solar_hour} and sw_in > {sw_in_threshold}")
            ddf_midday = ddf[(ddf['local_solar_hour'] >= min_solar_hour) & 
                             (ddf['local_solar_hour'] <= max_solar_hour) & 
                             (ddf['sw_in'] > sw_in_threshold)]
            
            return ddf_midday
            
        except Exception as time_err:
            print(f"Error processing time column: {time_err}")
            print("Falling back to sw_in threshold only")
            return ddf[ddf['sw_in'] > sw_in_threshold * 1.5]  # Higher threshold as proxy
            
    else:
        print("No timestamp column found. Using sw_in threshold only.")
        # If no time column is found, use a higher sw_in threshold as proxy for mid-day
        higher_threshold = sw_in_threshold * 1.5
        print(f"Using higher sw_in threshold ({higher_threshold}) as mid-day proxy")
        return ddf[ddf['sw_in'] > higher_threshold]


# =============================================================================
# Main GeoTIFF Creation Function (MODIFIED FOR MID-DAY FILTERING)
# =============================================================================
def create_sap_velocity_tif(csv_filepath, output_tif='sap_velocity_map.tif',
                            ...
                            dask_scheduler='processes', # Default to processes
                            use_midday_filter=True,  # NEW: Enable mid-day filtering
                            min_solar_hour=10,       # NEW: Start of mid-day window
                            max_solar_hour=14):      # NEW: End of mid-day window
    """
    Process sap velocity data using Dask for parallel aggregation and create a GeoTIFF map.
    Includes mid-day filtering based on local solar time approximated from longitude.
    ...
    """
    print(f"--- Starting GeoTIFF Creation Process (Dask Parallelized v2.2 - With Mid-day Filtering) ---") # Updated version marker
    print(f"Input CSV: {csv_filepath}")
    print(f"Output Target: {output_tif}")
    print(f"SW_in Threshold (>): {sw_in_threshold}")
    print(f"Mid-day Filtering: {use_midday_filter}")
    if use_midday_filter:
        print(f"   Local Solar Time Window: {min_solar_hour}:00 - {max_solar_hour}:00")
    ...
    try:
        ...
        # Extend dtypes to include timestamp columns if they exist
        dtypes = {'latitude': float, 'longitude': float, 'sw_in': float, 'sap_velocity_cnn_lstm': float}
        
        # Check for time columns in the sample
        sample_cols = pd.read_csv(sorted_filepath, nrows=1).columns
        time_cols = [col for col in sample_cols if any(time_str in col.lower() for time_str in ['time', 'date', 'utc'])]
        
        # Include time columns if they exist
        all_cols = list(dtypes.keys()) + time_cols
        
        # Update the date_format variable to match your timestamp format
        date_format = '%Y-%m-%d %H:%M:%S%z'  # Format with timezone offset

        # Then modify the dd.read_csv call
        ddf = dd.read_csv(
            sorted_filepath, 
            ...
            usecols=all_cols, 
            assume_missing=True,
            parse_dates=time_cols if time_cols else None,
            date_format=lambda x: pd.to_datetime(x, format=date_format, errors='coerce')
  )

        # Define Dask operations: filter, dropna, groupby, mean
        print("Filtering/Aggregating with Dask...")
        
        # Apply mid-day filtering if enabled
        if use_midday_filter:
            ddf_daytime = filter_for_midday(ddf, sw_in_threshold, min_solar_hour, max_solar_hour)
        else:
            # Original approach - just filter by sw_in
...
if __name__ == "__main__":
    ...
    output_geotiff = r'sap_velocity_map_global_midday_v1.tif'  # Changed filename to indicate mid-day filtering
    output_plot_png = r'sap_velocity_map_global_midday_v1.png'  # Changed filename to indicate mid-day filtering

    # Processing parameters
    DASK_BLOCKSIZE = '256MB'  # Adjust based on RAM/cores (e.g., '128MB', '512MB')
    DASK_SCHEDULER = 'threads'  # 'threads' on Windows to avoid multiprocessing spawn issues
    ...
    # Mid-day filtering parameters
    USE_MIDDAY_FILTER = True  # Enable the new mid-day filtering
    MIN_SOLAR_HOUR = 12  # Start of mid-day window (10:00 local solar time)
    MAX_SOLAR_HOUR = 14  # End of mid-day window (14:00 local solar time)

    # --- Execution ---
    print("===== Starting Script (Dask Parallelized v2.2 - With Mid-day Filtering) =====")  # Updated version marker
    ...
    if not os.path.exists(csv_file):
        ...
        try:
            ...
            # 2. Create the GeoTIFF (or Tiles) using Dask aggregation with mid-day filtering
            print("\n===== 2. Creating GeoTIFF (using Dask with Mid-day Filtering) =====")
            grid_data_result, extent_result, resolution_result = create_sap_velocity_tif(
                csv_filepath=csv_file, 
                output_tif=output_geotiff, 
                chunk_size=DASK_BLOCKSIZE,
                manual_resolution=MANUAL_RESOLUTION, 
                use_sparse=USE_SPARSE_IF_NOT_TILING,
                create_tiles=ENABLE_TILING, 
                tile_size=TILE_PIXEL_SIZE,
                sw_in_threshold=SW_IN_DAYTIME_THRESHOLD, 
                sorting_method=SORTING_METHOD,
                dask_scheduler=DASK_SCHEDULER,
                use_midday_filter=USE_MIDDAY_FILTER,  # Enable mid-day filtering
                min_solar_hour=MIN_SOLAR_HOUR,        # Define mid-day start
                max_solar_hour=MAX_SOLAR_HOUR         # Define mid-day end
            )
