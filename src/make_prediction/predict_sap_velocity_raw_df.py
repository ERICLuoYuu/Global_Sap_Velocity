...
Modified: Maps predictions to original (unscaled) dataframe
"""
import os
import sys
import argparse
...
def make_predictions_parallel(df, models, feature_columns, label_scaler, input_width=8, shift=1, n_workers=None, original_df=None):
    """
    ...
    MODIFIED to accept and return predictions mapped to the original DataFrame.

    Parameters:
    -----------
    df : pandas.DataFrame
        DataFrame containing the features (prepared/scaled data)
    models : dict
    ...
    original_df : pandas.DataFrame, optional
        Original unscaled DataFrame to map predictions onto

    Returns:
    --------
    pandas.DataFrame
        DataFrame with added prediction columns (mapped to original_df if provided)
    """
    ...
    # Store results - start with a copy of the original df if provided, else use the prepared df
    if original_df is not None:
        results_df = original_df.copy()
        logger.info("Using original (unscaled) DataFrame for mapping predictions")
    else:
        results_df = df.copy()
        logger.info("Original DataFrame not provided, using prepared DataFrame for mapping")
...
def make_predictions_improved(df, models, feature_columns, label_scaler, input_width=8, shift=1, original_df=None):
    """
...
def main():
    ...
    # Store original index and create a copy of original unscaled data
    original_index = df_raw.index.copy()
    logger.info(f"Original index type: {type(original_index)}")
    
    # IMPORTANT: Keep a copy of the original unscaled data for mapping predictions
    df_original = df_raw.copy()
    logger.info("Created copy of original unscaled data for prediction mapping")
    ...
    if args.parallel:
        ...
        # Pass the original unscaled dataframe for mapping
        df_predictions = make_predictions_parallel(
            ...
            original_df=df_original  # PASS ORIGINAL UNSCALED DATAFRAME
        )
        ...
        # Use original unscaled dataframe for mapping
        results_df = df_original.copy()  # USE ORIGINAL DATAFRAME
    ...
    # --- Final Steps ---
    # The predictions should already be mapped to the original dataframe
    ...
    # Save predictions
    output_file = output_dir / f"{input_file.stem}_predictions_raw.csv" # Simpler name
