...
data = pd.read_csv('./outputs/processed_data/merged/site/gap_filled_size1_hourly_after_filter/all_biomes_merged_data.csv').set_index('TIMESTAMP').sort_index()
data = add_time_features(data)
# Make sure to include the site column when selecting columns
# Assuming there is a 'site' column in the original data file
data = data[['sap_velocity', 'sw_in', 'ext_rad', 'ta', 'ws', 'vpd', 'ppfd_in', 'biome', 'site', 'Day sin', 'Week sin', 'Month sin', 'Year sin']].dropna()
...
# New site-based splitting strategy
training_portion = 0.8
training_data = pd.DataFrame()
test_data = pd.DataFrame()

# Group by site and split each site's data
for site, site_data in data.groupby('site'):
    # Sort data by timestamp (already done because of sort_index() above)
    # Split the data for this site: first 80% to training, rest to testing
    site_len = len(site_data)
    site_train = site_data.iloc[:int(site_len * training_portion)]
    site_test = site_data.iloc[int(site_len * training_portion):]
    
    # Append to our training and test dataframes
    training_data = pd.concat([training_data, site_train])
    test_data = pd.concat([test_data, site_test])

# Optional: Sort the combined data by timestamp
...
# Drop the site column before modeling if not needed as a feature
X = training_data.drop(columns=['sap_velocity', 'site'])
y = training_data['sap_velocity'].values
X_test = test_data.drop(columns=['sap_velocity', 'site'])
