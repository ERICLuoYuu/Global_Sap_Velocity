...
def _hour_processing_worker(args):
    """
    Worker function for processing a single hour with enhanced CPU and memory efficiency.
    ...
    dict
        Processing results
    """
    year, month, day, hour, variables, shapefile, output_dir, temp_dir, cpu_core = args
    ...
    if cpu_core is not None:
        try:
            ...
            if hasattr(os, 'sched_setaffinity'):
                ...
                print(f"Worker for hour {hour:02d} set to use CPU core {cpu_core}")
    ...
    try:
        ...
        # Calculate precise time range for this hour
        hour_start = pd.Timestamp(year=year, month=month, day=day, hour=hour)
...
class ERA5LandGEEProcessor:
    ...
    def read_shapefile(self, shapefile_path):
        ...
        try:
            ...
            traceback.print_exc()
            return None
    def get_era5land_variable_from_gee(self, variable, year, month, day=None, hour=None, scale=None, shapefile=None, region=None, local_file=None):
        """
        Access ERA5-Land data from local TIF files or Google Earth Engine for a specific time period.
        Fixed version with proper imports to avoid 'referenced before assignment' errors.
        ...
        scale : float, optional
            Scale in meters for the Earth Engine export
        shapefile : str, optional
            Path to shapefile for region definition
        region : dict or ee.Geometry, optional
            Region to extract data for
        local_file : str or Path, optional
            Path to local file containing the data (will use this instead of GEE)
        ...
        """
        # Import required libraries at the beginning to avoid 'referenced before assignment' errors
        import rasterio
        try:
            import rioxarray
            HAVE_RIOXARRAY = True
        except ImportError:
            HAVE_RIOXARRAY = False
            print("rioxarray not available, using fallback methods for TIF processing")
        ...
        # Get variable name to load
        gee_var = self.GEE_VARIABLE_MAPPING.get(variable, variable)
        
        # Check if local file is provided - use it instead of GEE
        if local_file is not None:
            local_file = Path(local_file)
            if local_file.exists():
                print(f"Loading {variable} for {time_str} from local file: {local_file}")
                
                try:
                    # Handle TIF file
                    suffix = local_file.suffix.lower()
                    if suffix in ['.tif', '.tiff', '.geotiff']:
                        # Multiple approaches to handle TIF files with fallbacks
                        ds = None
                        errors = []
                        
                        # Approach 1: Use rioxarray if available (preferred)
                        if HAVE_RIOXARRAY:
                            try:
                                print("Using rioxarray to open TIF file")
                                
                                # Use chunking for large files if dask is available
                                if HAVE_DASK:
                                    ds = rioxarray.open_rasterio(local_file, chunks={'x': 1024, 'y': 1024})
                                else:
                                    ds = rioxarray.open_rasterio(local_file)
                                
                                # If it's a DataArray, convert to Dataset with proper variable name
                                if isinstance(ds, xr.DataArray):
                                    ds = ds.to_dataset(name=gee_var)
                                
                                print(f"Successfully loaded {local_file} using rioxarray")
                            except Exception as rio_err:
                                errors.append(f"rioxarray approach failed: {str(rio_err)}")
                                ds = None  # Reset for next attempt
                        
                        # Approach 2: Use rasterio directly if rioxarray failed
                        if ds is None:
                            try:
                                print("Using rasterio directly to open TIF file")
                                
                                with rasterio.open(local_file) as src:
                                    # Read data
                                    data = src.read(1)
                                    
                                    # Get transform and create coordinates
                                    transform = src.transform
                                    height, width = data.shape
                                    
                                    # Generate lon/lat coordinates
                                    lons = np.zeros(width)
                                    lats = np.zeros(height)
                                    
                                    # Calculate center coordinates for each pixel
                                    for x in range(width):
                                        lon, _ = transform * (x + 0.5, 0)
                                        lons[x] = lon
                                    
                                    for y in range(height):
                                        _, lat = transform * (0, y + 0.5)
                                        lats[y] = lat
                                    
                                    # Create dataset
                                    ds = xr.Dataset(
                                        data_vars={gee_var: (('latitude', 'longitude'), data)},
                                        coords={'latitude': lats, 'longitude': lons}
                                    )
                                    
                                    print(f"Successfully loaded {local_file} using rasterio directly")
                            except Exception as r_err:
                                errors.append(f"rasterio approach failed: {str(r_err)}")
                                ds = None  # Reset for next attempt
                        
                        # Approach 3: Ultimate fallback - use matplotlib/PIL to read image
                        if ds is None:
                            try:
                                import matplotlib.pyplot as plt
                                print("Using matplotlib as fallback to read TIF file")
                                
                                # Read image using matplotlib
                                img = plt.imread(local_file)
                                height, width = img.shape[:2]
                                
                                # Create simple coordinate system (this is approximate)
                                # Assuming the file covers the region of interest
                                if region is not None:
                                    try:
                                        if isinstance(region, dict):
                                            ...
                                            # Try to get bounds from ee.Geometry
                                    ...
                                    # Use defaults if no region specified
                                    lon_min, lon_max = config.DEFAULT_LON_MIN, config.DEFAULT_LON_MAX
                                    lat_min, lat_max = config.DEFAULT_LAT_MIN, config.DEFAULT_LAT_MAX
                                
                                # Create coordinates
                                lons = np.linspace(lon_min, lon_max, width, endpoint=False)
                                lats = np.linspace(lat_max, lat_min, height, endpoint= False)  # Note reverse order for latitude
                                
                                # Create dataset
                                ds = xr.Dataset(
                                    data_vars={gee_var: (('latitude', 'longitude'), img)},
                                    coords={'latitude': lats, 'longitude': lons}
                                )
                                
                                print(f"Successfully loaded {local_file} using matplotlib")
                            except Exception as plt_err:
                                errors.append(f"matplotlib approach failed: {str(plt_err)}")
                        
                        # If all approaches failed, raise an exception with all errors
                        if ds is None:
                            raise ValueError(f"Failed to load TIF file using all approaches: {', '.join(errors)}")
                        
                        # Handle missing coordinates
                        for dim in ['latitude', 'longitude', 'lat', 'lon', 'x', 'y']:
                            if dim in ds.dims and dim not in ds.coords:
                                print(f"Creating missing {dim} coordinate")
                                ds[dim] = np.arange(ds.dims[dim])
                        
                        # Rename coordinates if needed for consistency
                        coord_mapping = {
                            'y': 'latitude',
                            'x': 'longitude',
                            'lat': 'latitude',
                        ...
                        ds = ds.rename({old: new for old, new in coord_mapping.items() 
                                        if old in ds.coords and new not in ds.coords})
                        
                        # Add a time coordinate if missing
                        if 'time' not in ds.coords:
                            # Create time value based on the target time
                            if hour is not None and day is not None:
                                time_value = pd.Timestamp(year=year, month=month, day=day, hour=hour)
                            elif day is not None:
                                time_value = pd.Timestamp(year=year, month=month, day=day)
                            else:
                                time_value = pd.Timestamp(year=year, month=month, day=1)
                            
                            # Add time coordinate - use expand_dims to preserve existing dimensions
                            ds = ds.expand_dims(time=[time_value])
                        
                        # Apply shapefile masking if needed
                        if shapefile is not None and HAVE_RIOXARRAY:
                            try:
                                if hasattr(ds, 'rio'):
                                    # Use rioxarray for masking
                                    import geopandas as gpd
                                    ...
                                    print(f"Applied shapefile mask from {shapefile}")
                                else:
                                    print("Dataset doesn't have rio accessor, skipping shapefile masking")
                            except Exception as mask_err:
                                print(f"Error applying shapefile mask: {mask_err}")
                        
                        # Set units metadata
                        if gee_var in ds.data_vars:
                            ds[gee_var].attrs['units'] = self._get_variable_units(gee_var)
                        
                        return ds
                    else:
                        print(f"File {local_file} is not a supported TIF format")
                
                except Exception as e:
                    print(f"Error loading from local file: {e}")
                    import traceback
                    traceback.print_exc()
                    print("Falling back to Google Earth Engine...")
            else:
                print(f"Local file not found: {local_file}")
                print("Falling back to Google Earth Engine...")
        
        # If no local file or local file loading failed, use Google Earth Engine
        print(f"Accessing ERA5-Land variable '{variable}' from Google Earth Engine for {time_str}")
        ...
        try:
            ...
            # Set up date range with hour precision if specified
            if hour is not None and day is not None:
            ...
            print(f"Found {collection_size} images in Google Earth Engine")
            
            if collection_size == 0:
                print("No images found in Google Earth Engine for the specified time period.")
                return None
            ...
            try:
                ...
                try:
                    ...
                    # Try to get sample to determine dimensions
                    ...
                    # Fallback to estimated dimensions
                    width = int(np.ceil((lon_max - lon_min) / (scale / 111320)))
                    height = int(np.ceil((lat_max - lat_min) / (scale / 111000)))
                    print(f"Fallback to estimated grid dimensions: {height}x{width}")
                
                # Create coordinate arrays
                lons = np.linspace(lon_min, lon_max, width, endpoint=False)  # Longitude increasing (West to East)
                lats = np.linspace(lat_max, lat_min, height, endpoint=False)  # Latitude decreasing (North to South)
                
                # Ensure coordinates are correctly sorted
                lons.sort()
                lats.sort()
                print(f"Coordinate ranges: lon={lons.min():.4f}-{lons.max():.4f}, lat={lats.min():.4f}-{lats.max():.4f}")
                ...
                for i in range(0, collection_size):
                    try:
                        ...
                        # PRIMARY METHOD: Export to temporary file
                        try:
                            ...
                            if temp_file.exists():
                                ...
                                # Small delay to ensure file system registers the file
                                import time
                                time.sleep(1)
                            ...
                            try:
                                ...
                                # Check for dimensions issue
                                if len(img_array.shape) > 2:
                                    # Handle 3D/4D arrays - take first band/channel
                                    if len(img_array.shape) == 3 and img_array.shape[2] == 1:
                                        img_array = img_array[:, :, 0]
                                    elif len(img_array.shape) == 3:
                                        img_array = img_array[:, :, 0]  # Take first channel
                                    elif len(img_array.shape) == 4:
                                        img_array = img_array[:, :, 0, 0]  # Take first band of first channel
                                    print(f"Reduced array dimensions from {len(img_array.shape)} to 2")
                                ...
                                if img_array.shape[:2] == (height, width):
                                    ...
                                    # Try to reshape
                                    try:
                                        img_array = np.resize(img_array, (height, width))
                                        all_arrays.append(img_array)
                                        timestamps.append(current_time)
                                        print(f"Resized array from {img_array.shape} to {(height, width)}")
                                    except:
                                        print(f"Skipping image {i+1} - shape mismatch: {img_array.shape} vs {(height, width)}")
                            except Exception as img_err:
                                print(f"All methods failed for image {i+1}: {str(img_err)}")
                ...
                if "different number of dimensions on data and dims" in str(ee_err):
                    print("Dimension mismatch detected. Attempting to fix dimensions...")
                    
                    # Try to handle dimension mismatches
                    try:
                        # Process the first image directly with dimension handling
                        first_img = ee.Image(image_list.get(0))
                        time_start = ee.Date(first_img.get('system:time_start')).format('YYYY-MM-dd HH:mm:ss').getInfo()
                        current_time = pd.to_datetime(time_start, utc=True)
                        
                        # Get image data with explicit dimension handling
                        img_array = geemap.ee_to_numpy(
                            first_img.select(gee_var),
                            region=region,
                            scale=scale
                        ...
                        if len(img_array.shape) > 2:
                            # Take first band/channel for multi-dimensional arrays
                            if len(img_array.shape) == 3 and img_array.shape[2] == 1:
                                img_array = img_array[:, :, 0]
                            elif len(img_array.shape) == 3:
                                img_array = img_array[:, :, 0]  # Take first channel
                        ...
                        da = xr.DataArray(
                            img_array[np.newaxis, :, :],  # Add time dimension
                            dims=['time', 'latitude', 'longitude'],
                            coords={
                                'time': [current_time],
                                'latitude': np.linspace(lat_max, lat_min, height),
                                'longitude': np.linspace(lon_min, lon_max, width)
                            },
                            name=gee_var
                        )
                        ...
                        # Add metadata
                        ds.attrs['title'] = f'ERA5-Land {gee_var} (single time point)'
                        ds.attrs['source'] = 'Google Earth Engine (dimension fix)'
                        ds[gee_var].attrs['units'] = self._get_variable_units(gee_var)
                        
                        print("WARNING: Fixed dimension mismatch but returning dataset with only a single time point")
                        return ds
                        
                    except Exception as fix_err:
                        print(f"Failed to fix dimension mismatch: {str(fix_err)}")
                
                # ULTIMATE FALLBACK: Try direct export to file then read
                try:
                    print("Attempting final fallback method: direct export to file...")
                    
                    # Export the first image as a test
                    first_img = ee.Image(image_list.get(0))
                    
                    # Create a temporary filename for the export
                    backup_file = self.temp_dir / f"{variable}_{year}_{month:02d}_backup.tif"
                    
                    # Export the image with a longer timeout
                    success = geemap.ee_export_image(
                        first_img.select(gee_var),
                    ...
                    if backup_file.exists():
                        print(f"Successfully exported backup file to {backup_file}")
                        
                        # Read the file using rasterio
                        try:
                            with rasterio.open(backup_file) as src:
                            ...
                            print(f"Failed to read backup file with rasterio: {src_err}")
                            # Try alternative read method
                            try:
                                import matplotlib.pyplot as plt
                                img_array = plt.imread(backup_file)
                                
                                # If it has more dimensions than expected, take first band
                                if len(img_array.shape) > 2:
                                    img_array = img_array[:, :, 0]
                                ...
                                print(f"Failed to read backup file with matplotlib: {alt_err}")
                                return None
                        ...
                        # Create a single-time dataset
                        da = xr.DataArray(
                            img_array[np.newaxis, :, :],  # Add time dimension manually
                            dims=['time', 'latitude', 'longitude'],
                            coords={
                                'time': [current_time],
                                'latitude': lats,
                        ...
                        # Add metadata
                        ds.attrs['title'] = f'ERA5-Land {gee_var} (single time point)'
                        ds.attrs['source'] = 'Google Earth Engine (fallback method)'
                        ds[gee_var].attrs['units'] = self._get_variable_units(gee_var)
                        
                        print("Warning: Returning dataset with only a single time point due to processing issues")
                        return ds
                    else:
                        print("Backup export method also failed")
                        return None
                
                except Exception as final_err:
                    print(f"All fallback methods failed: {str(final_err)}")
                    return None
    ...
    def load_variable_data(self, variables, year, month, day=None, hour=None, shapefile=None, data_dir=None):
        """
        Load and preprocess multiple ERA5-Land variables, optimized for local files when available.
        ...
        --------
        dict
            Dictionary where keys are variable names and values are the processed datasets
        """
        if not rioxarray:
        ...
        if data_dir is not None:
            ...
            if data_dir.exists() and data_dir.is_dir():
                print(f"Looking for local data files in {data_dir}")
                using_local_files = True
                
                # Search for files matching our variables and time period
                for var_name in variables:
                    # Look for NetCDF files first
                    nc_pattern = f"*{var_name}*{year}*{month:02d}*.nc*"
                    nc_files = list(data_dir.glob(nc_pattern))
                    
                    # Also look for GeoTIFF files
                    tif_pattern = f"*{var_name}*{year}*{month:02d}*.tif*"
                    tif_files = list(data_dir.glob(tif_pattern))
                    
                    # Combine results
                    all_files = nc_files + tif_files
                    
                    if all_files:
                        # Take the first matching file
                        local_files[var_name] = all_files[0]
                        print(f"Found local file for {var_name}: {all_files[0]}")
                    else:
                        print(f"No local file found for {var_name}")
        ...
        for var_name in variables:
            ...
            # 1. Get data from GEE (or local file) - MODIFIED to include hour parameter
            try:
                # Check if we have a local file for this variable
                local_file = local_files.get(var_name) if using_local_files else None
                ...
                print(f"  ERROR: Failed to load data for {var_name}: {gee_err}")
                ds = None
    ...
    def create_prediction_dataset(self, start_date=None, end_date=None, points=None, region=None, use_existing_data=True, use_utc=True):
        ...
        Uses vectorized approach for climate data lookup.
        Skip coordinates that don't have any valid data to save processing time.
        ...
        use_utc : bool, optional
            Whether to use UTC timezone for timestamps
        ...
        for var_name, ds in self.datasets.items():
            print(f"Dataset '{var_name}' contains variables: {list(ds.data_vars)}")
        ...
        if points:
            ...
            # Track valid and invalid points
            ...
            if hasattr(self, 'temp_climate_data') and hasattr(self, 'precip_climate_data'):
                try:
                    ...
                    # Get climate data for all points at once
                    ...
                    print("Successfully retrieved climate data for all points")
                except Exception as e:
                    print(f"Error in batch climate data retrieval: {str(e)}")
                    import traceback
                    traceback.print_exc()
                    climate_data_available = False
            else:
                climate_data_available = False
                print("Climate data not available - using default values")
            ...
            for i, point in enumerate(points):
                ...
                # Check if this point has valid data for all variables before processing
                has_valid_data = True
                
                # Quick check for valid data in each variable
                for var_name in prediction_vars:
                    try:
                        ...
                        if data_var:
                            # Extract the nearest grid cell for validity check
                            ts = ds[data_var].sel(
                                latitude=point['lat'],
                                longitude=point['lon'],
                                method='nearest'
                            ...
                            # Check if all values are NaN (no valid data)
                            if np.isnan(ts.values).all():
                                print(f"Point {point_data['name']} has no valid data for variable {var_name}. Skipping.")
                                has_valid_data = False
                                break
                    except Exception as e:
                        print(f"Error checking data for point {point_data['name']}: {str(e)}")
                        has_valid_data = False
                        break
                
                # Skip this point if it has no valid data
                ...
                if climate_data_available:
                    point_data['annual_mean_temperature'] = float(temps[i])
                    point_data['annual_precipitation'] = float(precips[i])
                else:
                    # Fallback to individual lookup or default values
                    if hasattr(self, 'temp_climate_data') and hasattr(self, 'precip_climate_data'):
                        try:
                            temp, precip = self.get_climate_at_location_direct(
                                point['lon'], 
                                point['lat'],
                                self.temp_climate_data,
                                self.precip_climate_data
                            )
                            point_data['annual_mean_temperature'] = temp
                            point_data['annual_precipitation'] = precip
                        except Exception as e:
                            print(f"Error getting climate for point {point_data['name']}: {str(e)}")
                            point_data['annual_mean_temperature'] = 15.0  # Default value
                            point_data['annual_precipitation'] = 800.0    # Default value
                    else:
                        point_data['annual_mean_temperature'] = 15.0  # Default value
                        point_data['annual_precipitation'] = 800.0    # Default value
            ...
            print(f"Processed {valid_points_count} valid points, skipped {invalid_points_count} invalid points")
        else:
            # --- GRID-BASED APPROACH ---
            # Track valid and invalid grid cells
            valid_cells_count = 0
            ...
            # --- Vectorized Climate Data Lookup for Grid ---
            if hasattr(self, 'temp_climate_data') and hasattr(self, 'precip_climate_data'):
                try:
                    ...
                    # Get climate data for all grid points at once
                    print(f"Getting climate data for {len(flat_lons)} grid points using vectorized approach...")
                    temps, precips = self.get_climate_at_location_direct(
                        flat_lons, flat_lats,
                        self.temp_climate_data,
                        self.precip_climate_data
                    ...
                    temp_grid = temps.reshape(lon_grid.shape)
                    precip_grid = precips.reshape(lon_grid.shape)
                    
                    climate_data_available = True
                    print("Successfully retrieved climate data for grid")
                except Exception as e:
                    print(f"Error in grid climate data retrieval: {str(e)}")
                    import traceback
                    traceback.print_exc()
                    climate_data_available = False
            else:
            ...
            for lat_idx, lat in enumerate(lats):
                for lon_idx, lon in enumerate(lons):
                    ...
                    # Check if this grid cell has valid data for all variables before processing
                    has_valid_data = True
                    ...
                    for var_name in prediction_vars:
                        try:
                            ...
                            if data_var:
                                # Extract the grid cell for validity check
                                ts = ds[data_var].sel(
                                    latitude=lat,
                                    longitude=lon,
                                    method='nearest'
                    ...
                    # Skip this grid cell if it has no valid data
                    if not has_valid_data:
                        invalid_cells_count += 1
                        # Print progress periodically
                        if processed_cells % max(1, int(total_cells / 10)) == 0:
                            print(f"Processed {processed_cells}/{total_cells} grid cells ({processed_cells/total_cells:.1%})")
                        continue
                    ...
                    # Get climate data for this grid cell
                    if climate_data_available:
                        grid_data['annual_mean_temperature'] = float(temp_grid[lat_idx, lon_idx])
                        grid_data['annual_precipitation'] = float(precip_grid[lat_idx, lon_idx])
                    else:
                        # Fallback to individual lookup or default values
                        if hasattr(self, 'temp_climate_data') and hasattr(self, 'precip_climate_data'):
                            try:
                                ...
                                grid_data['annual_mean_temperature'] = temp
                                grid_data['annual_precipitation'] = precip
                            except Exception as e:
                                print(f"Error getting climate for grid cell ({lat}, {lon}): {str(e)}")
                                grid_data['annual_mean_temperature'] = 15.0  # Default value
                                grid_data['annual_precipitation'] = 800.0    # Default value
                        else:
                            grid_data['annual_mean_temperature'] = 15.0  # Default value
                            grid_data['annual_precipitation'] = 800.0    # Default value
            ...
            print(f"Processed {valid_cells_count} valid grid cells, skipped {invalid_cells_count} invalid grid cells")
        ...
        if all_data:
            ...
            print("No data found for the specified time period or all coordinates had invalid data")
            return None


    def _has_valid_data(self, data_array):
        """
        Check if a data array contains any valid (non-NaN) values.
        
        Parameters:
        -----------
        data_array : numpy.ndarray or xarray.DataArray
            Data array to check for valid values
            
        Returns:
        --------
        bool
            True if the array contains at least one valid value, False otherwise
        """
        try:
            ...
            # Check for at least one non-NaN value
            return not np.isnan(data_array).all()
        except Exception:
            # In case of any error, assume no valid data
            return False
    ...
    def process_single_day(self, variables, year, month, day, shapefile=None, output_dir=None, 
                      hourly_parallel=False, num_workers=None, use_cpu_affinity=True, 
                      data_dir=None):
        """
        Process a single day of ERA5-Land data, enhanced with CPU affinity and local file support.
        ...
        hourly_parallel : bool, optional
            Whether to use hourly parallelization
        num_workers : int, optional
            Number of worker processes
        use_cpu_affinity : bool, optional
            Whether to use CPU affinity for workers
        data_dir : str or Path, optional
        ...
        try:
            # Load data with support for local files
    ...
    def _process_day_with_hourly_parallel(self, variables, year, month, day, 
                                    ...
                                    output_dir=None, use_cpu_affinity=True,
                                    data_dir=None):
        """
        Process a single day with hourly parallelization, enhanced with CPU core assignment.
        ...
        output_dir : str or Path, optional
            Output directory
        use_cpu_affinity : bool, optional
            Whether to assign specific CPU cores to workers
        data_dir : str or Path, optional
            Path to local data files
        ...
        print(f"Processing {year}-{month:02d}-{day:02d} using {num_workers} parallel processes")
        ...
        # Get available CPU cores if using CPU affinity
        cpu_cores = None
        if use_cpu_affinity:
            try:
                import psutil
                p = psutil.Process()
                cpu_cores = list(p.cpu_affinity())
                print(f"Available CPU cores: {cpu_cores}")
            except (ImportError, AttributeError):
                print("CPU affinity not available on this system")
                use_cpu_affinity = False
        
        # Prepare arguments for each hour
        process_args = []
        for hour in range(24):
            # Assign CPU core if enabled
            cpu_core = None
            if use_cpu_affinity and cpu_cores:
                cpu_core = cpu_cores[hour % len(cpu_cores)]
            
            # (year, month, day, hour, variables, shapefile, output_dir, temp_dir, cpu_core)
            process_args.append((year, month, day, hour, variables, shapefile, output_dir, temp_dir, cpu_core))
        ...
        if successful_results:
            ...
            try:
                ...
                if hourly_dfs:
                    # Optimize DataFrame combination for memory efficiency
                    # For large dataframes, combine in batches
                    try:
                        if sum(df.memory_usage(deep=True).sum() for df in hourly_dfs) > 1e9:  # > 1GB
                            print("Using memory-efficient DataFrame combination")
                            combined_df = None
                            batch_size = 4  # Combine 4 hours at a time
                            
                            for i in range(0, len(hourly_dfs), batch_size):
                                batch = hourly_dfs[i:i+batch_size]
                            ...
                            # For smaller data, concatenate directly
                            combined_df = pd.concat(hourly_dfs)
                    except Exception:
                        # Fall back to standard concat
                        combined_df = pd.concat(hourly_dfs)
                    
                    # Sort by timestamp and/or location
                    ...
                    # Optimize saving for large dataframes
                    if len(combined_df) > 1_000_000:
                        # Save in chunks to reduce memory usage
                        print(f"Saving large DataFrame in chunks")
                        chunk_size = 100_000
                        for i in range(0, len(combined_df), chunk_size):
                            mode = 'w' if i == 0 else 'a'
                            header = i == 0
                            combined_df.iloc[i:i+chunk_size].to_csv(
                                output_file, 
                                mode=mode, 
                                header=header, 
                                index=(combined_df.index.name is not None)
                            )
                    else:
                        # Standard save for smaller DataFrames
                        combined_df.to_csv(output_file, index=(combined_df.index.name is not None))
                ...
                print(f"Error combining hourly data: {e}")
                import traceback
                traceback.print_exc()
                return None
        else:
    ...
    def _process_month_with_hourly_parallel(self, variables, year, month, 
                                     ...
                                     use_cpu_affinity=True, data_dir=None):
        """
    ...
    def process_month_parallel(self, variables, year, month, shapefile=None, num_workers=None, 
                          ...
                          use_cpu_affinity=True, data_dir=None):
        """
        Process an entire month of ERA5-Land data with parallel processing.
        Enhanced with batch processing, CPU affinity, and local file support.
        ...
        hourly_parallel : bool, optional
            Whether to use hourly parallelization
        days_per_batch : int, optional
            Number of days to process in each batch
        use_cpu_affinity : bool, optional
            Whether to use CPU affinity for workers
        data_dir : str or Path, optional
            Path to local data files
        ...
        if hourly_parallel:
            # Process month with hourly parallelization
        ...
        # Original day-level parallelization
        # Determine days in month
        days_in_month = calendar.monthrange(year, month)[1]
        ...
        for batch_start in range(1, days_in_month + 1, days_per_batch):
            batch_end = min(batch_start + days_per_batch - 1, days_in_month)
            days_in_batch = batch_end - batch_start + 1
            
            print(f"\nProcessing batch: days {batch_start} to {batch_end}")
            
            # Prepare arguments for this batch
            process_args = []
            for day in range(batch_start, batch_end + 1):
                # (year, month, day, variables, shapefile, output_dir, processor)
                process_args.append((year, month, day, variables, shapefile, output_dir, self))
            
            # Adjust number of workers for this batch
            batch_workers = min(num_workers, days_in_batch)
            
            # Process the batch in parallel
            with mp.Pool(processes=batch_workers) as pool:
                batch_results = pool.map(_day_processing_worker, process_args)
            
            # Collect successful results
            batch_files = [r for r in batch_results if r is not None]
...
def main():
    """
    Main function for ERA5-Land processing with advanced optimization options.
    """
    import argparse
    ...
    parser.add_argument('--data-dir', type=str, help='Directory containing local data files')
    parser.add_argument('--no-cpu-affinity', action='store_true', help='Disable CPU core affinity for workers')
        ...
        if args.parallel or args.hourly_parallel:
            if args.day is not None:
                ...
                processor.process_single_day(
                    ...
                    use_cpu_affinity=not args.no_cpu_affinity,
