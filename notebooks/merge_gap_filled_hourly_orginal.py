...
def merge_sap_env_data_site(output_base_dir, daytime_only=False ):
        ...
        try:
            ...
            try:
                ...
                for date in unique_dates:
                    try:
                        ...
                        solar_pos = pvlib.solarposition.get_solarposition(times_of_day, lat, lon, altitude=site_elevation)
            ...
            if daytime_only:
                ...
                daytime_mask = calculate_daytime_mask(
                    ...
                    lat=lat, 
                    lon=lon, 
                    elevation=ele if not pd.isna(ele) else 0
                )
                n_before = len(df_hourly)
                df_hourly_for_daily = df_hourly[daytime_mask.values].copy()
                n_after = len(df_hourly_for_daily)
                print(f"  {location_type}: Filtered {n_before - n_after}/{n_before} nighttime hours ({(n_before - n_after)/n_before*100:.1f}%)")
            else:
            ...
            static_exact = [
                'latitude', 'longitude', 'elevation', 'slope', 'aspect_sin', 'aspect_cos',
                'mean_annual_temp', 'mean_annual_precip', 'temp_seasonality', 
                'precip_seasonality', 'canopy_height', 'stand_age'
            ]
            ...
            if 'total_precipitation_hourly_sum' in df_daily.columns and 'potential_evaporation_hourly_sum' in df_daily.columns:
        ...
        if biome_files:
            ...
            for bf in biome_files:
                ...
                for chunk in chunk_iter:
                    chunk.to_csv(all_biomes_path, mode='a' if not first_file else 'w', 
    ...
    if biome_merged_hourly:
        ...
        daily_all = save_biome_collections(biome_merged_daily, daily_out_dir, "merged_daily", skip_all_biomes_merge=False)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Output directory (default: paths.merged_data_root)'
    )
